...
const sendEmail = async ({ email, template, data }) => {
  try {
    // Skip sending emails in test environment or when credentials missing
    if (process.env.NODE_ENV === 'test') {
      console.log('[EmailService] Test mode - skipping email send');
      return { success: true, message: 'Skipped in test' };
    }
    if (!process.env.EMAIL_USER || !process.env.EMAIL_PASS) {
      console.warn('[EmailService] Email credentials not configured. Skipping email send in development mode.');
      return { success: true, message: 'Skipped - no credentials configured' };
    }
...
const sendNotificationEmail = async (email, subject, message) => {
  try {
    ...
    if (process.env.NODE_ENV === 'test') {
      console.log('[EmailService] Test mode - skipping notification email');
      return { success: true, message: 'Skipped in test' };
    }
    const transporter = createTransporter();
