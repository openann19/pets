    ...
    const matches = await Match.aggregate([
      { $match: query },
      {
        $addFields: {
          ...
          isBlockedByCurrentUser: {
            $cond: {
              if: { $eq: ['$user1', req.userId] },
              then: '$userActions.user1.isBlocked',
              else: '$userActions.user2.isBlocked'
            }
          },
          unreadCount: {
            $size: {
      ...
      },
      {
        $match: {
          ...
          isBlockedByCurrentUser: { $ne: true }
        }
      },
  ...
  try {
    ...
    if (otherUser.preferences.notifications.email && 
      ...
      try {
        ...
        logger.error('Message notification email error:', emailError);
      }
    }
...
const archiveMatch = async (req, res) => {
  try {
    ...
    logger.error('Archive match error:', error);
    res.status(500).json({
      success: false,
...
const blockMatch = async (req, res) => {
  try {
    ...
    // Directly set using Mongoose methods
    match.userActions[userKey] = match.userActions[userKey] || {};
    match.userActions[userKey].isBlocked = true;
    match.userActions[userKey].lastSeen = new Date();
    
    // Explicitly mark userActions as modified for Mongoose
    match.markModified('userActions');
    await match.save();
    ...
    logger.error('Block match error:', error);
    res.status(500).json({
      success: false,
...
const favoriteMatch = async (req, res) => {
  try {
    ...
    const currentFavorite = match.userActions[userKey]?.isFavorite || false;
    const newFavorite = !currentFavorite;
    
    // Directly set using Mongoose methods
    match.userActions[userKey] = match.userActions[userKey] || {};
    match.userActions[userKey].isFavorite = newFavorite;
    match.userActions[userKey].lastSeen = new Date();
    ...
    logger.error('Favorite match error:', error);
    res.status(500).json({
      success: false,
