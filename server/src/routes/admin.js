...
router.get('/security/alerts/download', checkPermission('security:read'), async (req, res) => {
  try {
    ...
    router.get('/users', checkPermission('users:read'), async (req, res) => {
      try {
        ...
        const formattedUsers = users.map(user => ({
          ...
          matchesCount: user.matchCount || 0
        }));
    ...
    router.post('/users/:id/suspend', checkPermission('users:write'), async (req, res) => {
      try {
        ...
        await logAdminActivity(req, 'SUSPEND_USER', { userId: req.params.id });
        ...
        logger.error('Failed to suspend user', { error, userId: req.params.id });
        res.status(500).json({
          success: false,
    ...
    router.post('/users/:id/unsuspend', checkPermission('users:write'), async (req, res) => {
      try {
        const user = await User.findById(req.params.id);
        if (!user) {
          return res.status(404).json({ success: false, message: 'User not found' });
        ...
        await logAdminActivity(req, 'UNSUSPEND_USER', { userId: req.params.id });
        ...
        logger.error('Failed to unsuspend user', { error, userId: req.params.id });
        res.status(500).json({
          success: false,
    ...
    router.post('/users/:id/ban', checkPermission('users:write'), async (req, res) => {
      try {
        const user = await User.findById(req.params.id);
        if (!user) {
          return res.status(404).json({ success: false, message: 'User not found' });
        ...
        await logAdminActivity(req, 'BAN_USER', { userId: req.params.id });
        ...
        logger.error('Failed to ban user', { error, userId: req.params.id });
        res.status(500).json({
          success: false,
    ...
    router.get('/chats', checkPermission('chats:read'), async (req, res) => {
      try {
        const {
          page = 1,
          limit = 50,
        ...
        // For now, return mock data until Chat model is implemented
        const mockChats = [
          {
            id: 'chat1',
            participants: [
        ...
        await logAdminActivity(req, 'VIEW_CHATS', { status, page, limit });
    ...
    router.post('/chats/:id/block', checkPermission('chats:write'), async (req, res) => {
      try {
        // Mock implementation until Chat model is implemented
        await logAdminActivity(req, 'BLOCK_CHAT', { chatId: req.params.id });
        ...
        logger.error('Failed to block chat', { error, chatId: req.params.id });
        res.status(500).json({
          success: false,
    ...
    router.post('/chats/:id/unblock', checkPermission('chats:write'), async (req, res) => {
      try {
        // Mock implementation until Chat model is implemented
        await logAdminActivity(req, 'UNBLOCK_CHAT', { chatId: req.params.id });
        ...
        logger.error('Failed to unblock chat', { error, chatId: req.params.id });
        res.status(500).json({
          success: false,
