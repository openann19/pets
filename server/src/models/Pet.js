...
petSchema.index({ location: '2dsphere' }); // Geospatial queries
petSchema.index({ owner: 1 }); // Owner lookup
petSchema.index({ species: 1, intent: 1 }); // Filtering by species and intent
petSchema.index({ breed: 1 }); // Breed filtering
petSchema.index({ isActive: 1, status: 1 }); // Active pet queries
petSchema.index({ 'featured.isFeatured': 1, 'featured.featuredUntil': 1 }); // Featured pets
petSchema.index({ createdAt: -1 }); // Recent pets
petSchema.index({ 'analytics.views': -1 }); // Popular pets
petSchema.index({ 'analytics.likes': -1 }); // Most liked pets
