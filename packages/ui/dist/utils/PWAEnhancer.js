...
export const useServiceWorker = () => {
    ...
    const registerServiceWorker = useCallback(async (swUrl = '/sw.js') => {
        if (!isSupported)
            return false;
        try {
...
export const useCacheManager = () => {
    ...
    const addToCache = useCallback(async (cacheName, requests) => {
        if (!isSupported)
            return false;
        try {
    ...
    const getCacheSize = useCallback(async (cacheName) => {
        if (!isSupported)
            return 0;
        try {
            const cache = await openCache(cacheName);
...
//# sourceMappingURL=PWAEnhancer.js.map