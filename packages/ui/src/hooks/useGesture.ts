...
export function useGesture(ref: React.RefObject<HTMLElement>, options: GestureOptions) {
  ...
  const triggerSound = useCallback((type: 'swipe' | 'tap' | 'success' = 'swipe') => {
    ...
    try {
      const W = window as Window & { webkitAudioContext?: typeof AudioContext } & Record<string, any>;
      const AC: typeof AudioContext | undefined = (W as any).AudioContext || W.webkitAudioContext;
      if (!AC) return;
  ...
  useEffect(() => {
    ...
    // Add event listeners
    element.addEventListener('touchstart', handleTouchStart as EventListener, { passive: false } as AddEventListenerOptions);
    element.addEventListener('touchmove', handleTouchMove as EventListener, { passive: false } as AddEventListenerOptions);
    element.addEventListener('touchend', handleTouchEnd as EventListener, { passive: false } as AddEventListenerOptions);
