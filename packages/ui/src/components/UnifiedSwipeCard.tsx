...
export function UnifiedSwipeCard({
  ...
  const triggerSound = useCallback((type: 'swipe' | 'hover' | 'tap' = 'swipe') => {
  ...
  const handleMouseMove = useCallback((event: React.MouseEvent) => {
    if (!enable3DTilt || !cardRef.current || !isCurrentCard) return;
  ...
  return (
    <motion.div
      ...
      whileHover={isCurrentCard && enableGlow ? { 
        scale: 1.02, 
        y: -5,
        transition: ANIMATIONS.spring.micro
      } : {}}
      aria-label={ariaLabel || `${data.name} profile card`}
      aria-describedby={ariaDescribedBy}
      role="button"
      ...
      >
        ...
        <AnimatePresence>
          ...
          <motion.div
            ...
            animate={{ opacity: likeOverlayOpacity, scale: 1 }}
          ...
          <motion.div
            ...
            initial={{ opacity: 0, scale: 0.8 }}
            animate={{ opacity: passOverlayOpacity, scale: 1 }}
            exit={{ opacity: 0, scale: 0.8 }}
          >
            <div className="flex items-center gap-2">
          ...
          <motion.div
            className="absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 z-20 px-6 py-3 rounded-full font-bold text-sm shadow-lg backdrop-blur-xl border border-white/20 text-white"
            style={{ 
            ...
            initial={{ opacity: 0, scale: 0.8 }}
            animate={{ opacity: superLikeOverlayOpacity, scale: 1 }}
            exit={{ opacity: 0, scale: 0.8 }}
          >
            <div className="flex items-center gap-2">
        ...
        <div className="relative w-full h-3/4 overflow-hidden">
          <img
            src={data.images[0] || '/placeholder-pet.jpg'}
            alt={data.name}
        ...
        <div className="absolute bottom-0 left-0 right-0 p-6 text-white">
          <div className="flex items-center justify-between mb-2">
            ...
            {data.age && <span className="text-lg opacity-80">{data.age} years old</span>}
          </div>
          
          {data.breed && (
            <p className="text-lg opacity-80 mb-2">{data.breed}</p>
          )}
          
          {data.distance && (
            <p className="text-sm opacity-60 mb-3">{data.distance} miles away</p>
          )}
          
          {data.description && (
            <p className="text-sm opacity-80 line-clamp-2">{data.description}</p>
          )}
        </div>
