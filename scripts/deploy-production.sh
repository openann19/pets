...
if [ ! -f "apps/web/.env.production" ]; then
    cp apps/web/.env.local apps/web/.env.production 2>/dev/null || echo "NEXT_PUBLIC_API_URL=http://localhost:5001/api" > apps/web/.env.production
...
PORT=5001 NODE_ENV=production nohup node server/server.js > server.log 2>&1 &
BACKEND_PID=$!
