...
class PWAOfflineService {
   ...
   */
  async cachePets(pets: Pet[]): Promise<void> {
    if (!this.db) return
   ...
   * Get cached pets
   */
  async getCachedPets(): Promise<CachedPet[]> {
    if (!this.db) return []
   ...
   */
  async queueAction(type: OfflineAction['type'], data: Record<string, unknown>): Promise<string> {
   ...
   */
  private async savePendingAction(action: OfflineAction): Promise<void> {
    if (!this.db) return
   ...
   */
  private async cleanupOldPets(): Promise<void> {
    if (!this.db) return
...
export function usePWAOffline() {
  ...
  const queueAction = async (type: OfflineAction['type'], data: Record<string, unknown>) => {
  ...
  const getCachedMessages = (chatId: string) => pwaOfflineService.getCachedMessages(chatId)
  const cachePets = (pets: Pet[]) => pwaOfflineService.cachePets(pets)
  const cacheMessages = (messages: Message[]) => pwaOfflineService.cacheMessages(messages)
