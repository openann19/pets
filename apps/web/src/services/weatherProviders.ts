...
export class WeatherProviders {
  ...
  static async fetchTomorrowIO(
    ...
    try {
      ...
      const response = await fetch(
        `${apiUrl}/timelines?location=${lat},${lon}&fields=${fields}&units=metric&apikey=${apiKey}`
      );
   ...
   */
  static async fetchVisualCrossing(
    apiKey: string,
    apiUrl: string,
    lat: number,
    lon: number
    ...
    try {
      const response = await fetch(
        `${apiUrl}/timeline/${lat},${lon}?unitGroup=metric&key=${apiKey}&include=current,days,hours,alerts,events`
      );

      if (!response.ok) throw new Error('Visual Crossing API request failed');
  ...
  static async fetchMeteomatics(
    ...
    try {
      ...
      const response = await fetch(
        `${apiUrl}/${now}/${parameters}/${lat},${lon}/json`,
        {
          headers: {
      ...
      if (!response.ok) throw new Error('Meteomatics API request failed');
   ...
   */
  private static mapTomorrowIOData(data: any): EnhancedWeatherData {
  ...
  // Helper methods
  private static getConditionFromCode(code: number): string {
  ...
  private static getDescriptionFromCode(code: number): string {
    return `Weather code ${code} - ${this.getConditionFromCode(code)}`;
  }

  private static getWeatherCodeFromConditions(conditions: string): number {
    ...
    for (const [key, value] of Object.entries(conditionMap)) {
      if (lowerConditions.includes(key)) return value;
    }
    return 1000;
  }

  private static calculateBeaufortScale(windSpeed: number): number {
    // Wind speed in m/s
  ...
  private static calculateTurbulence(gust: number, speed: number): 'calm' | 'light' | 'moderate' | 'severe' {
  ...
  private static getUVRiskLevel(uvIndex: number): string {
  ...
  private static getPrecipitationIntensity(rate: number): 'none' | 'light' | 'moderate' | 'heavy' | 'extreme' {
