...
class SuccessStoriesService {
   ...
   */
  private async getStoriesFromCMS(options: {
    limit: number
    featured?: boolean
    tags?: string[]
    cursor?: string
   ...
   */
  private async getStoriesFromDatabase(options: {
    limit: number
    featured?: boolean
    tags?: string[]
    cursor?: string
   ...
   */
  private getFallbackStories(limit: number): SuccessStoriesResponse {
...
export function useSuccessStories() {
  ...
  const fetchStories = async (options: Parameters<typeof successStoriesService.getSuccessStories>[0] = {}) => {
    setIsLoading(true)
    setError(null)
  ...
  const submitStory = async (story: Parameters<typeof successStoriesService.submitSuccessStory>[0]) => {
    setIsLoading(true)
    setError(null)
