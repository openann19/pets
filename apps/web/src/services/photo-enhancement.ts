...
class PhotoEnhancementService {
  ...
  async enhancePhoto(
    ...
    try {
      ...
      // Auto enhancements
      if (autoColor) transformations.push('e_auto_color')
      if (autoContrast) transformations.push('e_auto_contrast')
      if (autoBrightness) transformations.push('e_auto_brightness')
      if (autoSaturation) transformations.push('e_auto_saturation')
      ...
      // Effects
      if (radius > 0) transformations.push(`r_${radius}`)
      if (shadow) transformations.push('e_shadow:30,x_5,y_5')
      if (vignette) transformations.push('e_vignette:50')
  ...
  getOptimizedUrl(
    imageUrl: string,
    useCase: 'thumbnail' | 'profile' | 'gallery' | 'hero' | 'avatar'
  ): string {
    const presets = {
   ...
   */
  private buildCloudinaryUrl(imageUrl: string, transformations: string): string {
    if (!this.cloudName) {
   ...
   * Get file sizes for comparison
   */
  private async getFileSizes(originalUrl: string, enhancedUrl: string): Promise<{
    original: number
    enhanced: number
    reduction: number
  }> {
   ...
   */
  private getQualityScore(transformations: string[]): number {
    let score = 100

    // Deduct points for compression
    if (transformations.includes('q_auto')) score -= 5
    if (transformations.includes('q_eco')) score -= 15
    if (transformations.includes('q_low')) score -= 25

    // Add points for enhancements
    if (transformations.includes('e_auto_color')) score += 5
    if (transformations.includes('e_auto_contrast')) score += 5
    if (transformations.includes('e_auto_brightness')) score += 5
    if (transformations.includes('e_auto_saturation')) score += 5
...
export function usePhotoEnhancement() {
  ...
  const getOptimizedUrl = (imageUrl: string, useCase: Parameters<typeof photoEnhancementService.getOptimizedUrl>[1]) => {
