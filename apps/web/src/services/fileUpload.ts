...
class FileUploadService {
   ...
   */
  async uploadVoiceMessage(audioBlob: Blob, duration: number): Promise<UploadResult> {
   ...
   */
  private detectResourceType(file: File): 'image' | 'video' | 'audio' | 'raw' {
   ...
   */
  async deleteFile(publicId: string, resourceType: 'image' | 'video' | 'audio' | 'raw' = 'image'): Promise<void> {
    try {
      if (!this.apiKey) {
...
export const uploadImage = (file: File, folder?: string) =>
  fileUploadService.uploadImage(file, { folder: folder ?? undefined });

export const uploadVoiceMessage = (audioBlob: Blob, duration: number) =>
  fileUploadService.uploadVoiceMessage(audioBlob, duration);

export const uploadDocument = (file: File, folder?: string) =>
  fileUploadService.uploadDocument(file, { folder: folder ?? undefined });
