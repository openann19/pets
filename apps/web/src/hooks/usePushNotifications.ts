...
export function usePushNotifications(options: UsePushNotificationsOptions = {}) {
  ...
  const [manager, setManager] = useState<PushNotificationManager | null>(null);
  ...
  const initialize = useCallback(async () => {
    if (!manager || state.isInitializing) return;
  ...
  const requestPermission = useCallback(async () => {
    if (!manager) return 'denied' as NotificationPermission;
...
export function useNotificationSettings() {
  ...
  useEffect(() => {
    ...
    fetch('/api/notifications/settings', {
      ...
      .catch(err => console.error('[NotificationSettings] Error loading settings:', err));
