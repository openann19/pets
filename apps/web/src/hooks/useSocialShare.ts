...
export function useSocialShare() {
  ...
  const shareToSocial = async (platform: string, data: ShareData) => {
    setIsSharing(true)
  ...
  const shareToWhatsApp = async (data: ShareData) => {
  ...
  const shareToTelegram = async (data: ShareData) => {
    const text = encodeURIComponent(`${data.text} ${data.url}`)
  ...
  const getShareUrl = (platform: string, data: ShareData): string => {
