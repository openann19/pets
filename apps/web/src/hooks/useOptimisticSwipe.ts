...
export function useOptimisticSwipe(options: OptimisticSwipeOptions = {}) {
  const queryClient = useQueryClient();
  const { user } = useAuthStore();
  const { onSuccess, onError, showToast = true } = options;

  const swipeMutation = useMutation({
    mutationFn: async ({ petId, action }: SwipeAction) => {
      switch (action) {
        case 'like':
    ...
    onMutate: async ({ petId, action }) => {
      ...
      queryClient.setQueryData(['swipe-queue'], (old: ApiResponse<Pet[]> | undefined) => {
        if (!old?.data) return old;
        
        // Remove the swiped pet from the queue
        const updatedPets = old.data.filter((pet: Pet) => pet._id !== petId);
  ...
  const swipe = useCallback((petId: string, action: 'like' | 'pass' | 'superlike') => {
    if (!user) {
...
export function useSwipeQueue(filters?: SwipeFilters) {
  const queryClient = useQueryClient();
