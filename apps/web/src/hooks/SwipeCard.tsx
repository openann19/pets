...
const SwipeCard = ({
    ...
    const swipeTimeoutRef = React.useRef<NodeJS.Timeout | null>(null);
    ...
    const handleDragEnd = (_event: MouseEvent | TouchEvent | PointerEvent | React.MouseEvent | React.TouchEvent | React.PointerEvent, info: PanInfo): void => {
        const threshold = 100;
    ...
    // Get primary photo
    const primaryPhoto =
        pet.photos.find((photo) => photo.isPrimary === true) ||
        pet.photos[0];
                        ...
                        <div className="space-y-4">
                            ...
                            {(pet as Pet & { healthInfo?: { vaccinated: boolean, spayedNeutered: boolean } }).healthInfo && (
                                <div className="flex items-center space-x-4 text-sm text-white/70">
                                    {(pet as Pet & { healthInfo?: { vaccinated: boolean, spayedNeutered: boolean } }).healthInfo?.vaccinated && (
                                        <span className="flex items-center space-x-1">
                                            <span className="w-2 h-2 bg-emerald-400 rounded-full"></span>
                                            <span>Vaccinated</span>
                                        </span>
                                    )}
                                    {(pet as Pet & { healthInfo?: { vaccinated: boolean, spayedNeutered: boolean } }).healthInfo?.spayedNeutered && (
                                        <span className="flex items-center space-x-1">
