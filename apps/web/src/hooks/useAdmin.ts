...
export const useUpdateUser = () => {
  ...
  return useMutation({
    mutationFn: ({ userId, updates }: { userId: string; updates: Partial<User> }) =>
      adminApiService.updateUser(userId, updates),
    onSuccess: () => {
...
export const useCacheManagement = () => {
  ...
  const invalidateCache = useMutation({
    mutationFn: (pattern: string) => adminApiService.invalidateCache(pattern),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['admin', 'cache'] });
    },
  });
...
export const useSendNotification = () => {
  ...
  return useMutation({
    mutationFn: (request: NotificationRequest) => adminApiService.sendNotification(request),
    onSuccess: () => {
...
export const useSystemOperations = () => {
  ...
  const restartSystem = useMutation({
    mutationFn: (service: string) => adminApiService.restartSystem(service),
    onSuccess: () => {
  ...
  const createBackup = useMutation({
    mutationFn: (type: string) => adminApiService.createBackup(type),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['admin', 'system'] });
    },
  });
...
export const useBulkUserOperations = () => {
  ...
  return useMutation({
    mutationFn: ({ operation, userIds, data }: {
      operation: string;
      userIds: string[];
      data?: Record<string, unknown>;
    }) => adminApiService.bulkUserOperation(operation, userIds, data),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['admin', 'users'] });
      queryClient.invalidateQueries({ queryKey: ['admin', 'stats'] });
    },
 ...
 */
export const usePlatformAnalytics = (period: 'day' | 'week' | 'month' | 'year' = 'month') => {
  return useQuery({
...
export const useFeatureFlags = () => {
  ...
  const updateFeatureFlag = useMutation({
    mutationFn: ({ name, enabled }: { name: string; enabled: boolean }) =>
      adminApiService.updateFeatureFlag(name, enabled),
    onSuccess: () => {
...
export const useResolveSecurityAlert = () => {
  ...
  return useMutation({
    mutationFn: (alertId: string) => adminApiService.resolveSecurityAlert(alertId),
    onSuccess: () => {
...
export const useContentModeration = () => {
  ...
  const moderateContent = useMutation({
    mutationFn: ({ contentId, action, reason }: {
      contentId: string;
      action: 'approve' | 'reject';
      reason?: string;
    }) => adminApiService.moderateContent(contentId, action, reason),
    onSuccess: () => {
