...
export function useFavorites() {
    ...
    const addFavoriteMutation = useMutation({
        mutationFn: async (petId: string) => {
            const response = await http.post<AddFavoriteResponse>('/api/favorites', {
                petId,
            });
            return response;
        },
        onMutate: async (petId) => {
            ...
            queryClient.setQueryData<FavoritesResponse>(['favorites'], (old) => {
                ...
                const optimisticFavorite: Favorite = {
                    _id: `temp-${Date.now()}`,
                    userId: 'current-user',
                    petId: { _id: petId } as Pet, // Minimal data for optimistic update
    ...
    const removeFavoriteMutation = useMutation({
        mutationFn: async (petId: string) => {
            const response = await http.delete<RemoveFavoriteResponse>(
                `/api/favorites/${petId}`
            );
            return response;
        },
    ...
    const toggleFavorite = (petId: string, currentlyFavorited: boolean) => {
...
export function useFavoriteStatus(petId: string) {
    const { data, isLoading } = useQuery({
        ...
        queryFn: async () => {
            const response = await http.get<CheckFavoriteResponse>(
                `/api/favorites/check/${petId}`
            );
            return response;
        },
        enabled: !!petId,
