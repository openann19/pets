...
export const useNeuralNetwork = (config: NeuralNetworkConfig) => {
  ...
  const forward = useCallback((input: number[]): number[] => {
    if (!network) return [];
    ...
    for (let i = 0; i < network.weights.length; i++) {
      ...
      for (let j = 0; j < network.weights[i].length; j++) {
        ...
        // Apply activation
        const activation = activationFunctions[network.activation as keyof typeof activationFunctions] || activationFunctions.gelu;
  ...
  const analyzeComplexity = useCallback((text: string): number => {
    if (!network) return 0;
  ...
  const extractTextFeatures = useCallback((text: string): number[] => {
  ...
  const train = useCallback(async (trainingData: { input: string; target: number }[]) => {
    if (!network) return;
