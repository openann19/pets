...
export const useSwipe = (): UseSwipeReturn => {
  ...
  const loadPets = useCallback(async () => {
    ...
    try {
      ...
      logger.error('Failed to load pets', err as Error, { page });
  ...
  const swipePet = useCallback(async (petId: string, action: 'like' | 'pass' | 'superlike'): Promise<SwipeResult> => {
    try {
      ...
      logger.error('Failed to swipe pet', err as Error, { petId, action });
