...
export const useGlobalTodos = () => {
  ...
  // Search todos
  const searchTodos = useCallback((keyword: string) => {
  ...
  const getTodosByCategory = useCallback((category: string) => {
    return todos.filter(todo => todo.category === category);
  }, [todos]);

  // Filter todos by priority
  const getTodosByPriority = useCallback((priority: 'high' | 'medium' | 'low') => {
...
const getDefaultTodos = (): TodoItem[] => {
  return [
    {
      id: 'todo-1',
