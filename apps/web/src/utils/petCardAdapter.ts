...
export const adaptPetToCardData = (pet: Pet): PetCardData => {
  ...
  const distanceKm = (pet as any).owner?.location !== undefined ? 2.5 : 0; // Placeholder distance
  ...
  return {
    ...
    breed: (pet as any).breed ?? 'Mixed',
    age: (pet as any).age,
    size: (pet as any).size as 'tiny' | 'small' | 'medium' | 'large' | 'extra-large',
    distanceKm,
    bio: (pet as any).description ?? 'Super friendly and loves to play!',
    photos,
    compatibility,
    gender: (pet as any).gender,
    species: (pet as any).species as 'dog' | 'cat' | 'bird' | 'rabbit' | 'other',
  };
};
...
export const generateMockPetCardData = (): PetCardData => {
  ...
  return {
    ...
    name: names[Math.floor(Math.random() * names.length)] || 'Buddy',
    breed: breeds[Math.floor(Math.random() * breeds.length)] || 'Mixed',
    age: Math.random() * 10 + 1,
    size: sizes[Math.floor(Math.random() * sizes.length)] || 'medium',
    distanceKm: Math.random() * 20 + 1,
    bio: bios[Math.floor(Math.random() * bios.length)] || 'Super friendly!',
    photos: [
