...
export class WebSocketManager {
  ...
  private dispatchConnectionEvent(status: 'connected' | 'disconnected' | 'reconnected' | 'failed') {
  ...
  private log(level: 'info' | 'warn' | 'error' | 'debug' | 'success', message: string) {
    ...
    switch (level) {
      ...
      case 'debug':
        if (process.env.NODE_ENV === 'development') {
          console.debug(`${prefix} ${message}`, { timestamp });
        }
        break;
      case 'success':
        console.log(`%c${prefix} ${message}`, 'color: green', { timestamp });
        break;
      default:
...
export const getWebSocketManager = (config?: WebSocketConfig) => 
  WebSocketManager.getInstance(config);