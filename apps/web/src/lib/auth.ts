 ...
 */
export async function verifyAuth(request: NextRequest): Promise<Partial<AuthPayload>> {
    try {
 ...
 */
export async function createAuthToken(payload: Omit<AuthPayload, 'iat' | 'exp'>): Promise<string> {
    const secret = new TextEncoder().encode(JWT_SECRET);
 ...
 */
export async function validateTwoFactor(_userId: string, token: string): Promise<boolean> {
    try {
