...
interface LeaderboardProps {
  ...
  category: 'overall' | 'streak' | 'matches' | 'engagement';
...
export const Leaderboard = ({
  ...
  const handleCategoryChange = (cat: string) => {
    setSelectedCategory(cat as typeof category);
  ...
  return (
    <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-lg border border-gray-200 dark:border-gray-700 overflow-hidden">
      ...
      {/* Top 3 Podium */}
      {entries.length >= 3 && (
        <div className="p-6 bg-gradient-to-b from-gray-50 to-white dark:from-gray-700/50 dark:to-gray-800">
        ...
        {entries.slice(3).map((entry, index) => (
          <motion.div
            ...
            className={`p-4 hover:bg-gray-50 dark:hover:bg-gray-700/50 transition-colors ${
              entry.isCurrentUser ? 'bg-pink-50 dark:bg-pink-900/20 border-l-4 border-pink-500' : ''
            }`}
          >
            <div className="flex items-center space-x-4">
              {/* Rank */}
              <div
                className={`w-12 h-12 rounded-full bg-gradient-to-r ${getRankColor(entry.rank)} flex items-center justify-center flex-shrink-0`}
              >
