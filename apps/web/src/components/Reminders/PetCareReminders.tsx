...
export interface PetCareReminder {
  ...
  title: string;
  description?: string;
  type: 'medication' | 'feeding' | 'grooming' | 'exercise' | 'vaccination' | 'checkup' | 'other';
  date: string;
  time?: string;
...
interface PetCareRemindersProps {
  reminders: PetCareReminder[];
  pets: Pet[];
  onAddReminder?: (reminder: Omit<PetCareReminder, 'id'>) => void;
  onUpdateReminder?: (reminder: PetCareReminder) => void;
  onDeleteReminder?: (reminderId: string) => void;
  onToggleComplete?: (reminderId: string, completed: boolean) => void;
}

export const PetCareReminders = ({
  reminders,
  pets,
  onAddReminder,
  ...
  onDeleteReminder,
  onToggleComplete,
}: PetCareRemindersProps) => {
  const [showAddModal, setShowAddModal] = useState(false);
  const [filterPet, setFilterPet] = useState<string>('all');
  const [filterType, setFilterType] = useState<string>('all');
  const [newReminder, setNewReminder] = useState<Omit<PetCareReminder, 'id'>>({
    petId: pets[0]?.id || '',
    title: '',
    description: '',
    type: 'other',
  ...
  const remindersByDate = filteredReminders.reduce(
    (acc, reminder) => {
    ...
    {} as Record<string, PetCareReminder[]>,
  );
  ...
  const handleAddReminder = () => {
    if (!newReminder.title || !newReminder.petId) return;
  ...
  const handleToggleComplete = (reminder: PetCareReminder) => {
  ...
  const getReminderTypeIcon = (type: PetCareReminder['type']) => {
    switch (type) {
      case 'medication':
        return '💊';
      case 'feeding':
  ...
  const getReminderTypeColor = (type: PetCareReminder['type']) => {
    switch (type) {
      case 'medication':
        return 'bg-blue-500';
      case 'feeding':
  ...
  const getFrequencyText = (reminder: PetCareReminder) => {
      ...
      <div className="px-6 py-4 border-b border-gray-200 dark:border-gray-700">
        <div className="flex items-center justify-between">
          ...
          <button
            ...
            className="px-3 py-1.5 bg-pink-600 hover:bg-pink-700 text-white text-sm font-medium rounded-lg flex items-center transition-colors"
          >
        ...
        <div className="flex flex-wrap items-center gap-3">
          <div className="flex items-center">
            ...
            <select
              ...
              className="text-sm border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-700 text-gray-900 dark:text-white py-1 px-2"
            >
      ...
      <div className="px-6 py-4">
        {sortedDates.length > 0 ? (
                ...
                <div className="space-y-2">
                  {remindersByDate[date]?.map((reminder) => {
                    ...
                    return (
                      <div
                        key={reminder.id}
                        className={`p-4 rounded-lg border ${reminder.completed
                      ...
                      >
                        <div className="flex items-center mb-2">
                          ...
                          <button
                            ...
                            className={`w-5 h-5 rounded border flex-shrink-0 ${reminder.completed
                          ...
                          {/* Type indicator */}
                          <div
                            className={`ml-3 w-8 h-8 rounded-full ${getReminderTypeColor(reminder.type)} flex items-center justify-center text-white`}
                          >
                          ...
                          {/* Pet avatar */}
                          {pet && (
                            <div className="ml-2 w-8 h-8 rounded-full bg-gray-200 dark:bg-gray-700 flex items-center justify-center overflow-hidden">
                              {pet.avatar ? (
                                <img
                                  src={pet.avatar}
                                  alt={pet.name}
                          ...
                          <div className="ml-3 flex-1">
                            <h4
                              className={`font-medium ${reminder.completed
                        ...
                        {reminder.description && (
                          <div
                            className={`pl-11 ml-3 text-sm ${reminder.completed
          ...
          <div className="text-center py-8">
            ...
            <button
              onClick={() => setShowAddModal(true)}
              className="px-4 py-2 bg-pink-600 hover:bg-pink-700 text-white font-medium rounded-lg inline-flex items-center transition-colors"
            >
              ...
              >
                ...
                <div className="mb-4">
                  ...
                  <input
                    ...
                    className="w-full px-4 py-2 border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-700 text-gray-900 dark:text-white"
                    placeholder="e.g., Give medication, Monthly grooming"
                    required
                  />
                </div>
                ...
                <div className="mb-4">
                  ...
                  <select
                    ...
                    onChange={(e) =>
                      setNewReminder({
                        ...newReminder,
                        type: e.target.value as PetCareReminder['type'],
                      })
                    }
                    className="w-full px-4 py-2 border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-700 text-gray-900 dark:text-white"
                  >
                ...
                <div className="mb-4">
                  ...
                  <select
                    ...
                    onChange={(e) =>
                      setNewReminder({
                        ...newReminder,
                        frequency: e.target.value as PetCareReminder['frequency'],
                      })
                    }
                    className="w-full px-4 py-2 border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-700 text-gray-900 dark:text-white"
                  >
                  ...
                  <div className="mb-4 grid grid-cols-2 gap-2">
                    <div>
                      ...
                      <input
                        ...
                        id="interval"
                        min="1"
                        value={newReminder.customFrequency?.interval || 1}
                        onChange={(e) =>
                          setNewReminder({
                            ...newReminder,
                            customFrequency: {
                    ...
                    <div>
                      ...
                      <select
                        id="unit"
                        value={newReminder.customFrequency?.unit || 'days'}
                        onChange={(e) =>
                          setNewReminder({
                            ...newReminder,
                            customFrequency: {
                    ...
                    {[30, 60, 1440].map((minutes) => (
                      ...
                      >
                        <input
                          type="checkbox"
                          id={`notify-${minutes}`}
                          checked={newReminder.notifyBefore.includes(minutes)}
                          onChange={(e) => {
                            if (e.target.checked) {
                              setNewReminder({
                                ...newReminder,
                                notifyBefore: [...newReminder.notifyBefore, minutes].sort(
                                  (a, b) => a - b,
                                ),
                              });
                            } else {
                ...
                <div className="mb-4">
                  <CustomTextarea
                    ...
                    onChange={(e: React.ChangeEvent<HTMLTextAreaElement>) => setNewReminder({ ...newReminder, description: e.target.value })}
                    rows={3}
                    variant="outline"
                    size="medium"
                    autoGrow={true}
