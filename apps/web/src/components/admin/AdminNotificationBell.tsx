...
interface ContentFlaggedData {
    ...
    violatedCategories: Array<{ category: string; score: number; threshold: number }>;
    provider: string;
    timestamp: string;
}
...
export function AdminNotificationBell({ userId, isAdmin }: AdminNotificationBellProps) {
    ...
    useEffect(() => {
        ...
        socketInstance.on('new-report', (data: NewReportData) => {
        ...
        socketInstance.on('content-flagged', (data: ContentFlaggedData) => {
            const notification: Notification = {
        ...
        socketInstance.on('user-action', (data: UserActionData) => {
            const notification: Notification = {
                id: `action-${Date.now()}`,
                type: 'user-action',
                                        ...
                                        >
                                            <div className="flex items-start gap-3">
                                                <div className="flex-shrink-0">
                                                    ...
                                                    {notification.type === 'content-flagged' && (
