...
export function AnnouncementProvider({ children }) {
  ...
  return (
    <AnnouncementContext.Provider value={{ announce, announcement }}>
      {children}
      {announcement && (
        <AriaLiveRegion priority={announcement.priority}>{announcement.message}</AriaLiveRegion>
      )}
...
export function SkipLink({ href = '#main-content', children = 'Skip to main content' }) {
  return (
    <a
      href={href}
      className="absolute z-50 bg-blue-600 text-white p-3 -translate-y-full focus:translate-y-0 transition-transform"
    >
      {children}
    </a>
  );
