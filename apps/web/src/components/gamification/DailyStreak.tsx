...
export const DailyStreak = ({ currentStreak, longestStreak, lastCheckIn }: DailyStreakProps) => {
  const isActiveToday = lastCheckIn
    ? new Date(lastCheckIn).toDateString() === new Date().toDateString()
    : false;

  const getStreakLevel = (streak: number) => {
    if (streak >= 30) return { level: 'Legend', color: 'from-purple-500 to-pink-500' };
    if (streak >= 14) return { level: 'Master', color: 'from-orange-500 to-red-500' };
    if (streak >= 7) return { level: 'Pro', color: 'from-yellow-500 to-orange-500' };
    if (streak >= 3) return { level: 'Rising', color: 'from-green-500 to-teal-500' };
  ...
  return (
    <div className="bg-white dark:bg-gray-800 rounded-2xl p-6 shadow-lg border border-gray-200 dark:border-gray-700">
      <div className="flex items-center justify-between mb-4">
        ...
        <div
          className={`px-3 py-1 rounded-full bg-gradient-to-r ${color} text-white text-xs font-semibold`}
        >
          {level}
        </div>
      </div>
      ...
      <div className="flex items-center justify-center mb-6">
        ...
        >
          <div
            className={`w-32 h-32 rounded-full bg-gradient-to-r ${color} flex items-center justify-center shadow-xl`}
          >
            <div className="bg-white dark:bg-gray-800 w-28 h-28 rounded-full flex flex-col items-center justify-center">
              <FireIcon
                className={`w-12 h-12 bg-gradient-to-r ${color} bg-clip-text text-transparent`}
              />
      ...
      <div className="mt-6">
        ...
        <div className="w-full bg-gray-200 dark:bg-gray-700 rounded-full h-2">
          <motion.div
            initial={{ width: 0 }}
            animate={{ width: `${(currentStreak % 7) * (100 / 7)}%` }}
