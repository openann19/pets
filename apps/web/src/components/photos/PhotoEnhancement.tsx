      ...
      <div className="relative group">
        ...
        {enhancedUrl && (
          <motion.div
            initial={{ opacity: 0 }}
            animate={{ opacity: 1 }}
            className="absolute top-2 right-2 bg-green-500 text-white px-2 py-1 rounded-full text-xs font-medium flex items-center space-x-1"
          >
            ...
            <div className="space-y-3">
              ...
              <div className="grid grid-cols-2 gap-3">
                {[
                  ...
                  <label key={key} className="flex items-center space-x-2">
                    <input
                      type="checkbox"
                      checked={enhancementOptions[key as keyof PhotoEnhancementOptions] as boolean}
              ...
              <div>
                ...
                <select
                  ...
                  onChange={(e) => setEnhancementOptions(prev => ({
                    ...prev,
                    quality: e.target.value as PhotoEnhancementOptions['quality']
                  }))}
...
function PhotoComparison({ originalUrl, enhancedUrl, className = '' }: PhotoComparisonProps) {
    ...
    <div className={`${className}`}>
      ...
      <div className="relative">
        <AnimatePresence mode="wait">
          <motion.div
            key={activeTab}
            initial={{ opacity: 0, x: activeTab === 'enhanced' ? 20 : -20 }}
            animate={{ opacity: 1, x: 0 }}
            exit={{ opacity: 0, x: activeTab === 'enhanced' ? -20 : 20 }}
            transition={{ duration: 0.2 }}
          >
            <img
              src={activeTab === 'enhanced' ? enhancedUrl : originalUrl}
              alt={`${activeTab} photo`}
...
// Auto-Enhancement Wrapper for Pet Photos
interface AutoEnhancePetPhotoProps {
  imageUrl: string
  petId: string
  className?: string
  onEnhanced?: (enhancedUrl: string) => void
...
export function AutoEnhancePetPhoto({ 
  imageUrl, 
  petId, 
  className = '',
  onEnhanced 
}: AutoEnhancePetPhotoProps) {
  return (
    <PhotoEnhancement
      imageUrl={imageUrl}
