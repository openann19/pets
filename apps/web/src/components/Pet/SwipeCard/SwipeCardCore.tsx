...
interface SwipeCardCoreProps {
  ...
  dragConstraints?: boolean | { top?: number; left?: number; right?: number; bottom?: number; } | React.RefObject<Element>;
}

export const SwipeCardCore = ({
  pet,
  onSwipe,
  onCardClick,
  style,
  ...
  const superlikeButtonRef = useRef<HTMLButtonElement>(null);
  const likeButtonRef = useRef<HTMLButtonElement>(null);
  
  const [isExiting, setIsExiting] = useState(false);
  const [focusedAction, setFocusedAction] = useState<'pass' | 'superlike' | 'like' | null>(null);
   ...
   */
  const handleButtonFocus = (action: 'pass' | 'superlike' | 'like') => {
  ...
  return (
    <>
      ...
      <motion.div
        ref={cardRef}
        className="relative w-full h-full cursor-grab active:cursor-grabbing focus:outline-none focus:ring-4 focus:ring-purple-500 focus:ring-opacity-50 rounded-2xl"
        style={{
          ...(style || {}),
          x,
          y,
        ...
        tabIndex={0}
        role="article"
        aria-label={`${pet.name}, ${pet.breed}, ${pet.age} years old. Use arrow keys to swipe: left to pass, right to like, up for superlike.`}
      >
        {/* Card Content */}
          ...
          <div className="p-4 h-1/4 flex flex-col justify-between">
            ...
            <div className="flex justify-center space-x-4 mt-2" role="group" aria-label="Swipe actions">
              ...
              <button
                ...
                onFocus={() => handleButtonFocus('pass')}
                ...
                className={`w-12 h-12 bg-red-500 text-white rounded-full flex items-center justify-center hover:bg-red-600 transition-all transform hover:scale-110 ${
                  focusedAction === 'pass' ? 'ring-4 ring-red-300 scale-110' : ''
                }`}
                aria-label={`Pass on ${pet.name}. Keyboard shortcut: Left arrow`}
                title="Pass (←)"
              >
              ...
              <button
                ...
                onFocus={() => handleButtonFocus('superlike')}
                onBlur={handleButtonBlur}
                onKeyDown={(e) => {
                  if (e.key === 'Enter' || e.key === ' ') {
                    e.preventDefault();
                ...
                className={`w-12 h-12 bg-blue-500 text-white rounded-full flex items-center justify-center hover:bg-blue-600 transition-all transform hover:scale-110 ${
                  focusedAction === 'superlike' ? 'ring-4 ring-blue-300 scale-110' : ''
                }`}
                aria-label={`Super like ${pet.name}. Keyboard shortcut: Up arrow`}
                title="Super Like (↑)"
              >
                <span className="text-2xl" aria-hidden="true">⭐</span>
              </button>
              ...
              <button
                ...
                onFocus={() => handleButtonFocus('like')}
                onBlur={handleButtonBlur}
                onKeyDown={(e) => {
                  if (e.key === 'Enter' || e.key === ' ') {
                    e.preventDefault();
                ...
                className={`w-12 h-12 bg-green-500 text-white rounded-full flex items-center justify-center hover:bg-green-600 transition-all transform hover:scale-110 ${
                  focusedAction === 'like' ? 'ring-4 ring-green-300 scale-110' : ''
                }`}
                aria-label={`Like ${pet.name}. Keyboard shortcut: Right arrow`}
                title="Like (→)"
              >
                <span className="text-2xl" aria-hidden="true">♥</span>
              </button>
        ...
        {focusedAction && (
          <div className="absolute bottom-24 left-1/2 transform -translate-x-1/2 bg-black bg-opacity-75 text-white px-4 py-2 rounded-lg text-sm whitespace-nowrap pointer-events-none">
