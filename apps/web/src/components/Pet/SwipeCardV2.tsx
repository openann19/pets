...
interface SwipeCardV2Props {
  pet: PetCardData;
  onSwipe: (action: 'like' | 'pass' | 'superlike', petId: string) => void;
  onCardClick?: () => void;
  className?: string;
...
const getGenderIcon = (gender?: 'male' | 'female'): string => {
  return gender === 'male' ? '♂' : gender === 'female' ? '♀' : '';
};

const _getSpeciesEmoji = (species?: string): string => {
  switch (species) {
    case 'dog': return '🐕';
    case 'cat': return '🐱';
    case 'bird': return '🐦';
...
// ===== Sound Effects =====
const playSound = (type: 'pop' | 'swipe' | 'match' = 'pop') => {
...
// ===== Main Component =====
const SwipeCardV2: React.FC<SwipeCardV2Props> = ({
  pet,
  onSwipe,
  onCardClick,
  className = '',
  ...
  const handleDragEnd = useCallback((_: MouseEvent | TouchEvent | PointerEvent, info: PanInfo) => {
    ...
    if (yOffset < -superThreshold && Math.abs(offset) < 50) {
      ...
      if (soundEffects) playSound('match');
      setIsExiting(true);
    ...
    // Strong swipe or drag beyond threshold
      ...
      if (soundEffects) playSound('swipe');
      setIsExiting(true);
  ...
  const handlePassClick = useCallback((e: React.MouseEvent) => {
    e.stopPropagation();
    handleButtonClick('pass');
  }, [handleButtonClick]);

  const handleSuperLikeClick = useCallback((e: React.MouseEvent) => {
    e.stopPropagation();
  ...
  const handlePhotoClick = useCallback((e: React.MouseEvent, index: number) => {
    e.stopPropagation();
    setCurrentPhotoIndex(index);
  }, []);

  const createPhotoClickHandler = useCallback((index: number) => {
    return (e: React.MouseEvent) => handlePhotoClick(e, index);
  }, [handlePhotoClick]);
  ...
  return (
    <div className={`relative mx-auto ${className}`} style={{ maxWidth: '420px' }}>
      {/* Swipe Overlays */}
      <motion.div
      ...
      <motion.div
        className="absolute top-6 left-1/2 transform -translate-x-1/2 z-20 px-5 py-2 rounded-full font-bold text-sm shadow-lg backdrop-blur-xl border border-white/20 text-white"
        style={{ 
      ...
      <motion.article
        ...
        className="relative rounded-2xl overflow-hidden shadow-lg bg-white dark:bg-neutral-900 cursor-grab active:cursor-grabbing"
        style={{ 
          x, 
          y, 
          rotate, 
      ...
      >
          ...
          {pet.photos.length > 1 && (
            <div className="absolute top-4 right-4 flex space-x-1">
              {pet.photos.map((_, index) => (
                <button
                  key={index}
                  onClick={createPhotoClickHandler(index)}
        ...
        <section className="p-4 space-y-1 text-neutral-900 dark:text-neutral-50">
          ...
          {/* Compatibility Badge */}
          {pet.compatibility !== undefined && pet.compatibility > 0 && (
            <div className="inline-flex items-center gap-1 rounded-full bg-emerald-600/10 text-emerald-700 dark:text-emerald-400 px-2 py-0.5 text-xs font-medium">
              <span>★</span>
        ...
        <div className="flex justify-center gap-6 py-3 pb-safe">
          ...
          <motion.button
            ...
            aria-label="Pass"
            className="rounded-full p-4 shadow-md bg-white dark:bg-neutral-800 hover:scale-105 active:scale-95 transition text-neutral-400 hover:text-neutral-600 dark:hover:text-neutral-300"
          ...
          <motion.button
            ...
            className="rounded-full p-4 shadow-md bg-white dark:bg-neutral-800 hover:scale-105 active:scale-95 transition text-sky-500 hover:text-sky-600"
            transition={{ type: 'spring', stiffness: 300, damping: 20 }}
          >
          ...
          <motion.button
            ...
            aria-label="Like"
            className="rounded-full p-4 shadow-md bg-white dark:bg-neutral-800 hover:scale-105 active:scale-95 transition text-rose-500 hover:text-rose-600"
            transition={{ type: 'spring', stiffness: 300, damping: 20 }}
          >
