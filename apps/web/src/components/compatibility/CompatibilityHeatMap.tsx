...
export function CompatibilityHeatMap({
  ...
  const overallScore = Math.round(
    Object.values(compatibilityData).reduce((sum, score) => sum + score, 0) / 5
  )
  ...
  return (
    <div className={`bg-white rounded-xl shadow-lg p-6 ${className}`}>
      ...
      <div className="grid grid-cols-5 gap-4 mb-6">
        {factors.map((factor) => {
          const score = compatibilityData[factor.key as keyof CompatibilityData]
          ...
          return (
            ...
            >
              <div
                className="aspect-square rounded-lg flex items-center justify-center text-white font-bold text-sm transition-all duration-200"
      ...
      {selectedFactor && (
        ...
        >
          ...
          <p className="text-sm text-gray-700">
            {getFactorDescription(selectedFactor, compatibilityData[selectedFactor as keyof CompatibilityData])}
          </p>
        </motion.div>
      )}
      ...
      <div className="space-y-3">
        ...
        {getCompatibilityInsights(compatibilityData).map((insight, index) => (
          <motion.div
            key={index}
      ...
      <div className="mt-6 p-4 bg-gradient-to-r from-pink-50 to-purple-50 rounded-lg">
        ...
        <ul className="text-sm text-gray-700 space-y-1">
          {getRecommendations(compatibilityData).map((recommendation, index) => (
...
function getFactorDescription(factor: string, score: number): string {
  ...
  return descriptions[factor as keyof typeof descriptions]?.[level] || 'Compatibility analysis available.'
}

function getCompatibilityInsights(data: CompatibilityData): string[] {
...
function getRecommendations(data: CompatibilityData): string[] {
...
export function CompactCompatibilityHeatMap({
  compatibilityData,
  className = ''
}: {
  compatibilityData: CompatibilityData
  ...
  return (
    <div className={`bg-white rounded-lg p-4 shadow-sm ${className}`}>
      ...
      <div className="grid grid-cols-5 gap-2">
        {Object.entries(compatibilityData).map(([key, score]) => (
