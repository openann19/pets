...
export const PrivacyControls = ({ settings: initialSettings, onUpdate }: PrivacyControlsProps) => {
  const [settings, setSettings] = useState<PrivacySettings>(initialSettings);
  const [isSaving, setIsSaving] = useState(false);

  const handleToggle = async (key: keyof PrivacySettings, value: unknown) => {
        ...
        <div className="p-4 bg-purple-50 dark:bg-purple-900/20 border border-purple-200 dark:border-purple-800 rounded-xl">
          <div className="flex items-center justify-between">
            ...
            <button
              ...
              disabled={isSaving}
              className={`relative inline-flex h-6 w-11 items-center rounded-full transition-colors ${settings.incognitoMode ? 'bg-purple-600' : 'bg-gray-300 dark:bg-gray-600'
                }`}
            >
              <span
                className={`inline-block h-4 w-4 transform rounded-full bg-white transition-transform ${settings.incognitoMode ? 'translate-x-6' : 'translate-x-1'
                  }`}
              />
            </button>
          </div>
      ...
      <div className="bg-white dark:bg-gray-800 rounded-2xl p-6 shadow-lg border border-gray-200 dark:border-gray-700">
        ...
        <div className="space-y-3">
          {[
            ...
            <button
              key={option.value}
              onClick={() => handleToggle('profileVisibility', option.value)}
              className={`w-full text-left p-4 rounded-xl border-2 transition-all ${settings.profileVisibility === option.value
      ...
      <div className="bg-white dark:bg-gray-800 rounded-2xl p-6 shadow-lg border border-gray-200 dark:border-gray-700">
        ...
        <div className="space-y-4">
          ...
          <ToggleItem
            ...
            onChange={(checked) => handleToggle('showLastActive', checked)}
            disabled={isSaving}
          />
          <ToggleItem
            ...
            onChange={(checked) => handleToggle('showReadReceipts', checked)}
            disabled={isSaving}
          />
        </div>
      </div>
      ...
      <div className="bg-white dark:bg-gray-800 rounded-2xl p-6 shadow-lg border border-gray-200 dark:border-gray-700">
        ...
        <div className="space-y-4">
          <ToggleItem
            ...
            onChange={(checked) => handleToggle('shareLocation', checked)}
            disabled={isSaving}
          />
          <ToggleItem
            label="Show Distance"
            ...
            onChange={(checked) => handleToggle('showDistance', checked)}
            disabled={isSaving || !settings.shareLocation}
          />
        </div>
      </div>
      ...
      <div className="bg-white dark:bg-gray-800 rounded-2xl p-6 shadow-lg border border-gray-200 dark:border-gray-700">
        ...
        <div className="space-y-3">
          {[
            ...
            <button
              key={option.value}
              onClick={() => handleToggle('allowMessages', option.value)}
              className={`w-full text-left p-4 rounded-xl border-2 transition-all ${settings.allowMessages === option.value
                  ? 'border-pink-500 bg-pink-50 dark:bg-pink-900/20'
                  : 'border-gray-200 dark:border-gray-700 hover:border-gray-300 dark:hover:border-gray-600'
                }`}
            >
...
const ToggleItem = ({ label, description, checked, onChange, disabled }: ToggleItemProps) => {
  return (
    <div className="flex items-center justify-between">
      ...
      <button
        ...
        disabled={disabled}
        className={`relative inline-flex h-6 w-11 items-center rounded-full transition-colors disabled:opacity-50 ${checked ? 'bg-pink-600' : 'bg-gray-300 dark:bg-gray-600'
          }`}
      >
        <span
