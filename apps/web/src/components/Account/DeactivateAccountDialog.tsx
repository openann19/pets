...
import type { AccountDeactivationRequest, AccountDeactivationResponse } from '@pawfectmatch/core/types';
...
export const DeactivateAccountDialog: React.FC<DeactivateAccountDialogProps> = ({
  ...
  const handleDeactivate = async (): Promise<void> => {
    ...
    try {
      ...
      const response: AccountDeactivationResponse = await accountService.deactivateAccount(request, token);
      if (!response.success) {
        throw new Error(response.message || 'Failed to deactivate account');
      }
      alert(
        `Account deactivated successfully at ${new Date(response.deactivatedAt).toLocaleString()}. ` +
      ...
      <DialogContentComponent className="max-w-lg bg-white dark:bg-gray-800 rounded-2xl p-6 shadow-lg border border-gray-200 dark:border-gray-700">
        ...
        <div className="space-y-2 mt-4">
          ...
          <Textarea
            ...
            placeholder="Taking a break, found a match, need time off, etc..."
            rows={3}
          />
        </div>
        {error && (
