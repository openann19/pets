...
// Real API implementations for AccountService and types
...
class AccountService {
  ...
  async requestAccountDeletion(args: AccountDeletionRequest, token: string): Promise<{ deletionScheduledDate: string; gracePeriodEndsAt: string; confirmationId: string }> {
...
type DeletionStep = 'warning' | 'confirm' | 'export' | 'final';
...
export function DeleteAccountDialog({
  ...
  const [reason, setReason] = useState<AccountDeletionRequest['reason']>('other');
  ...
  const handleDeleteAccount = async (): Promise<void> => {
    ...
    try {
      ...
      alert(
        `Account deletion scheduled for ${new Date(response.deletionScheduledDate).toLocaleDateString()}. ` +
  ...
  const renderWarningStep = (): React.ReactElement => (
    <>
      <DialogHeaderComponent>
  ...
  const renderExportStep = (): React.ReactElement => (
    <>
      <DialogHeaderComponent>
    ...
    <>
      ...
      <div className="space-y-4 py-4">
        <div className="space-y-2">
          ...
          <Select
            value={reason}
            onValueChange={(value: AccountDeletionRequest['reason']) => setReason(value)}
          >
            <SelectTrigger>
              <SelectValue />
            </SelectTrigger>
        ...
        <div className="space-y-2">
          ...
          {confirmEmail && confirmEmail !== userEmail && (
            <p className="text-sm text-red-500">Email does not match</p>
          )}
        </div>
      ...
      <DialogFooterComponent className="gap-2">
        ...
        <Button
          ...
          disabled={isProcessing || !confirmEmail || confirmEmail !== userEmail}
        >
