...
interface Post {
  ...
  location?: string;
  tags: string[];
  sentiment: 'happy' | 'training' | 'adventure' | 'relaxed' | 'playful';
}
...
export default function HomeFeed({
  ...
  const [currentMediaIndex, setCurrentMediaIndex] = useState<{ [postId: string]: number }>({});
  ...
  const toggleVideoPlay = useCallback((postId: string, mediaId: string) => {
  ...
  const nextMedia = useCallback((postId: string) => {
  ...
  const prevMedia = useCallback((postId: string) => {
    setCurrentMediaIndex(prev => {
      const post = feedPosts.find(p => p.id === postId);
      if (!post) return prev;
    ...
    <div className={`space-y-6 ${className}`}>
      {feedPosts.map((post, index) => (
        <PostCard
          ...
          currentMediaIndex={currentMediaIndex[post.id] || 0}
          isVideoPlaying={playingVideos.has(post.media[currentMediaIndex[post.id] || 0]?.id)}
        />
      ))}
...
interface PostCardProps {
  ...
  onShare: (postId: string) => void;
  onBookmark: (postId: string) => void;
  onVideoToggle: (postId: string, mediaId: string) => void;
  onNextMedia: (postId: string) => void;
  onPrevMedia: (postId: string) => void;
      ...
      <div className="relative">
        {currentMedia.type === 'photo' ? (
          ...
          <div className="relative w-full aspect-square bg-black">
            ...
            {/* Video controls */}
            <button
              onClick={() => onVideoToggle(post.id, currentMedia.id)}
        ...
        {post.media.length > 1 && (
          <div className="absolute top-4 right-4 flex space-x-1">
            {post.media.map((_, index) => (
              <button
                key={index}
                onClick={() => setCurrentMediaIndex(prev => ({ ...prev, [post.id]: index }))}
        ...
        {post.media.length > 1 && (
          <>
            <button
              onClick={() => onPrevMedia(post.id)}
              className="absolute left-4 top-1/2 transform -translate-y-1/2 w-8 h-8 bg-black/30 rounded-full flex items-center justify-center text-white hover:bg-black/50 transition-colors"
            >
              ←
            </button>
            <button
              onClick={() => onNextMedia(post.id)}
              className="absolute right-4 top-1/2 transform -translate-y-1/2 w-8 h-8 bg-black/30 rounded-full flex items-center justify-center text-white hover:bg-black/50 transition-colors"
            >
              →
            </button>
          </>
            ...
            >
              <div className="flex items-center space-x-3">
                <input
                  ...
                  className="flex-1 px-3 py-2 bg-neutral-100 dark:bg-neutral-800 rounded-full text-sm focus:outline-none focus:ring-2 focus:ring-primary-500"
                  onKeyPress={(e) => {
                    if (e.key === 'Enter') {
                      handleComment();
                    }
                ...
                <button
                  ...
                  className="px-4 py-2 bg-primary-500 text-white rounded-full text-sm font-semibold hover:bg-primary-600 disabled:opacity-50 disabled:cursor-not-allowed transition-colors"
                >
                  Post
                </button>
              </div>
