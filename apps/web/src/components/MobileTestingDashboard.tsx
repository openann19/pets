...
export function MobileTestingDashboard({ className = '' }: MobileTestingDashboardProps) {
  const { results, isRunning, progress, runTests } = useMobileTesting();
  const [quickCheck, setQuickCheck] = useState<{ score: number; issues: string[] } | null>(null);
  const [activeTab, setActiveTab] = useState<'overview' | 'touch' | 'responsive' | 'performance' | 'accessibility' | 'safe-areas'>('overview');
...
// Tab Components
function OverviewTab({ results }: { results: MobileTestResults }) {
  const categories = [
    {
      name: 'Touch Targets',
      ...
      <div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {categories.map((category) => (
            <div key={category.name} className="bg-gray-50 dark:bg-gray-800 rounded-lg p-4">
...
function TouchTargetsTab({ results }: { results: TouchTargetTest[] }) {
  return (
      ...
      <div className="space-y-2">
        {results.map((test, index) => (
          <div key={index} className="flex items-center justify-between p-3 bg-gray-50 dark:bg-gray-800 rounded-lg">
            ...
            {test.recommendation && (
              <p className="text-sm text-red-600 dark:text-red-400 max-w-xs text-right">
                {test.recommendation}
              </p>
            )}
          </div>
...
function ResponsiveTab({ results }: { results: ResponsiveTest[] }) {
  return (
    <div className="space-y-4">
...
function PerformanceTab({ results }: { results: PerformanceTest[] }) {
  return (
    <div className="space-y-4">
...
function AccessibilityTab({ results }: { results: AccessibilityTest[] }) {
...
function SafeAreasTab({ results }: { results: SafeAreaTest[] }) {
  return (
    <div className="space-y-4">
