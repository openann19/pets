...
export function SubscriptionManager(): JSX.Element {
  ...
  useEffect(() => {
    ...
    const premiumPlans: PricingPlan[] = [
      ...
      },
      {
        ...
        ...(billingInterval === 'yearly' ? { popular: true, savings: 'Save 25%' } : { popular: true }),
        stripePriceId:
          billingInterval === 'monthly' ? 'price_premium_monthly' : 'price_premium_yearly',
        features: [
          '✨ Unlimited swipes',
          '🎯 AI-powered matching',
      ...
      },
      {
        ...
        currency: 'USD',
        interval: billingInterval,
        ...(billingInterval === 'yearly' ? { savings: 'Save 25%' } : {}),
        stripePriceId:
          billingInterval === 'monthly' ? 'price_ultimate_monthly' : 'price_ultimate_yearly',
        features: [
  ...
  const loadSubscription = async (): Promise<void> => {
    try {
      ...
      if (response && response.success && response.data) {
        setCurrentSubscription(response.data.subscription as Subscription);
      } else {
        setCurrentSubscription(null);
      }
      logger.info('Subscription loaded', { status: response?.data?.subscription?.status });
    } catch (error) {
  ...
  const loadUsageStats = async (): Promise<void> => {
    try {
      const response = await api.subscription.getUsageStats();
      if (response.success && response.data) {
        setUsageStats(response.data as unknown as UsageStats);
      }
    } catch (error) {
  ...
  const handleSubscribe = async (plan: PricingPlan): Promise<void> => {
    ...
    try {
      ...
      } else if (result && 'sessionId' in result && result.sessionId) {
        // Fallback to Stripe.js if URL not provided
        const stripe = await loadStripe();
        if (stripe) {
          interface StripeWithRedirect {
            redirectToCheckout: (options: { sessionId: string }) => Promise<{ error?: { message: string } }>;
          }
          await (stripe as unknown as StripeWithRedirect).redirectToCheckout({ sessionId: result.sessionId });
        }
      }
  ...
  const handleCancelSubscription = async (): Promise<void> => {
    ...
    try {
      const response = await api.subscription.cancelSubscription(currentSubscription.id);
  ...
  const handleReactivateSubscription = async (): Promise<void> => {
    ...
    try {
      const response = await api.subscription.reactivateSubscription(currentSubscription.id);
  ...
  const loadStripe = async (): Promise<unknown> => {
    ...
    <div className="max-w-7xl mx-auto px-6 py-12">
      ...
      >
        ...
        <p className="text-gray-600">
          Find your pet&apos;s perfect match faster with AI-powered features and unlimited
          possibilities
        </p>
      </motion.div>
          ...
          <div className="flex items-center justify-between flex-wrap gap-4">
            <div>
              <div className="flex items-center mb-2">
                ...
                <h2 className="text-2xl font-bold">{currentSubscription.plan.name} Member</h2>
              </div>
            ...
            <div className="flex gap-3">
              {currentSubscription.cancelAtPeriodEnd ? (
                <button
                  ...
                  className="px-6 py-3 bg-white text-purple-600 rounded-xl font-semibold hover:bg-gray-100 transition-colors disabled:opacity-50"
                ...
                <>
                  <button
                    ...
                    className="px-6 py-3 bg-white/20 backdrop-blur text-white rounded-xl font-semibold hover:bg-white/30 transition-colors"
                  ...
                  <button
                    ...
                    disabled={isProcessing}
                    className="px-6 py-3 bg-white/10 backdrop-blur text-white rounded-xl font-semibold hover:bg-white/20 transition-colors disabled:opacity-50"
        ...
        >
          <div className="bg-white rounded-2xl p-6 shadow-lg">
            ...
            <div className="mt-2 h-2 bg-gray-200 rounded-full overflow-hidden">
              <div
                ...
                style={{ width: `${(usageStats.swipesUsed / usageStats.swipesLimit) * 100}%` }}
              />
            </div>
          </div>
          ...
          <div className="bg-white rounded-2xl p-6 shadow-lg">
            ...
            <div className="mt-2 h-2 bg-gray-200 rounded-full overflow-hidden">
              <div
                ...
                style={{
                  width: `${(usageStats.superLikesUsed / usageStats.superLikesLimit) * 100}%`,
                }}
              />
            </div>
          </div>
          ...
          <div className="bg-white rounded-2xl p-6 shadow-lg">
            ...
            <div className="mt-2 h-2 bg-gray-200 rounded-full overflow-hidden">
              <div
                ...
                style={{ width: `${(usageStats.boostsUsed / usageStats.boostsLimit) * 100}%` }}
              />
            </div>
          </div>
          ...
          <div className="bg-white rounded-2xl p-6 shadow-lg">
            ...
            <p className="text-xs text-gray-500 mt-2">
              {currentSubscription?.plan.id === 'premium' ||
              currentSubscription?.plan.id === 'ultimate'
                ? '+35% with Premium'
        ...
        <div className="bg-gray-100 rounded-full p-1 flex">
          <button
            ...
            className={`px-6 py-2 rounded-full font-semibold transition-all ${
              billingInterval === 'monthly' ? 'bg-white text-gray-900 shadow-md' : 'text-gray-600'
            }`}
          ...
          <button
            ...
            className={`px-6 py-2 rounded-full font-semibold transition-all ${
              billingInterval === 'yearly' ? 'bg-white text-gray-900 shadow-md' : 'text-gray-600'
            }`}
      ...
      >
        ...
        <div className="grid md:grid-cols-3 gap-6">
          <div className="text-center">
            ...
            <h2 className="font-semibold text-lg mb-2">AI-Powered Matching</h2>
            <p className="text-gray-600">
          ...
          <div className="text-center">
            ...
            <h2 className="font-semibold text-lg mb-2">3x More Matches</h2>
            <p className="text-gray-600">
          ...
          <div className="text-center">
            ...
            <h2 className="font-semibold text-lg mb-2">Priority Support</h2>
            <p className="text-gray-600">Get help from our expert team within minutes, not hours</p>
          </div>
        </div>
      </motion.div>
            ...
            >
              <div className="text-center">
                ...
                <button
                  ...
                  aria-label="Close subscription success message"
                >
                  Start Exploring
                </button>
              </div>
