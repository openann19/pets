...
// Types for our playground locations
export interface PetPlayground {
  id: string;
  name: string;
  description: string;
  address: string;
...
interface PlaygroundMapProps {
  ...
  initialZoom?: number;
  playgrounds?: PetPlayground[];
  onPlaygroundSelect?: (playground: PetPlayground) => void;
...
export const PlaygroundMap = ({
  ...
  const handleFilterChange = (category: keyof PlaygroundFilters, key: string, value: boolean) => {
    const newFilters = {
      ...filters,
      [category]: {
  ...
  const handleSingleFilterChange = <T extends number | boolean>(key: keyof PlaygroundFilters, value: T) => {
    const newFilters = {
      ...filters,
      [key]: value,
    };
  ...
  const handlePlaygroundClick = (playground: PetPlayground) => {
  ...
  const getPlaygroundTypeColor = (type: PetPlayground['type']) => {
    switch (type) {
      case 'dog-park':
        return 'bg-green-500';
      case 'pet-friendly-cafe':
  ...
  return (
    <div className="bg-white dark:bg-gray-800 rounded-xl shadow-lg border border-gray-200 dark:border-gray-700 overflow-hidden h-[600px] relative">
      {/* Map container */}
      <div
        ref={mapRef}
        className="w-full h-full bg-gray-100 dark:bg-gray-700 relative"
        style={{
      ...
      >
        ...
        {mapLoaded &&
          playgrounds.map((playground) => (
            ...
            >
              <div
                className={`w-8 h-8 rounded-full ${getPlaygroundTypeColor(playground.type)} flex items-center justify-center text-white shadow-md group-hover:scale-110 transition-transform z-10`}
              >
      ...
      <div className="absolute top-4 right-4 flex flex-col space-y-2">
        ...
        <button
          ...
          className={`w-10 h-10 bg-white dark:bg-gray-800 rounded-full flex items-center justify-center shadow-lg hover:bg-gray-100 dark:hover:bg-gray-700 transition-colors ${showFilters ? 'text-pink-500' : 'text-gray-600 dark:text-gray-400'}`}
      ...
      {/* Filter panel */}
      <div
        className={`absolute top-0 right-0 h-full w-80 bg-white dark:bg-gray-800 shadow-lg border-l border-gray-200 dark:border-gray-700 transform transition-transform duration-300 ${showFilters ? 'translate-x-0' : 'translate-x-full'
          } overflow-y-auto`}
      >
        <div className="p-4">
              ...
              {Object.keys(filters.types).map((type) => (
                ...
                >
                  <input
                    ...
                    className="w-4 h-4 text-pink-600 rounded border-gray-300 focus:ring-pink-500 dark:border-gray-600 dark:bg-gray-700"
                  />
                  <label
                    htmlFor={`type-${type}`}
                  ...
                  >
                    <span className="mr-2">
                      {getPlaygroundTypeIcon(type as PetPlayground['type'])}
                    </span>
                    {getReadableTypeName(type as PetPlayground['type'])}
                  </label>
                </div>
              ))}
            </div>
          ...
          <div className="mb-6">
            ...
            <div className="flex items-center space-x-2">
              <input
                ...
                onChange={(e) => handleSingleFilterChange('ratings', parseInt(e.target.value))}
            ...
            <div className="flex items-center justify-between mb-2">
              ...
              <button
                ...
                onClick={() => {
                  ...
                  const newAmenities: typeof filters.amenities = {
                    water: true,
                    fenced: true,
                    shade: true,
                    seating: true,
              ...
              {Object.keys(filters.amenities).map((amenity) => (
                ...
                >
                  <input
                    type="checkbox"
                    id={`amenity-${amenity}`}
                    checked={filters.amenities[amenity as keyof typeof filters.amenities]}
          ...
          <div className="mb-6">
            ...
            <div className="flex items-center space-x-2">
              <input
                ...
                onChange={(e) => handleSingleFilterChange('distance', parseInt(e.target.value))}
                className="w-full h-2 bg-gray-200 rounded-lg appearance-none cursor-pointer dark:bg-gray-700"
              />
          ...
          <div className="mb-6">
            <div className="flex items-center">
              <input
                ...
                onChange={(e) => handleSingleFilterChange('favoritesOnly', e.target.checked)}
                className="w-4 h-4 text-pink-600 rounded border-gray-300 focus:ring-pink-500 dark:border-gray-600 dark:bg-gray-700"
              />
              <label
      ...
      {selectedPlayground && (
        <div className="absolute left-0 bottom-0 right-0 bg-white dark:bg-gray-800 border-t border-gray-200 dark:border-gray-700 p-4">
          <div className="flex items-start justify-between">
            <div>
              <div className="flex items-center space-x-2 mb-1">
                <span
                  className={`px-2 py-1 rounded-full text-xs text-white ${getPlaygroundTypeColor(selectedPlayground.type)}`}
                >
              ...
              <div className="flex flex-wrap gap-1">
                {selectedPlayground.amenities.slice(0, 4).map((amenity, index) => (
                  <span
                    key={index}
                    className="px-2 py-0.5 bg-gray-100 dark:bg-gray-700 text-gray-800 dark:text-gray-300 text-xs rounded"
                  >
                    {amenity}
                  </span>
                ))}
                {selectedPlayground.amenities.length > 4 && (
            ...
            <button
              onClick={() => setSelectedPlayground(null)}
              className="text-gray-500 hover:text-gray-700 dark:text-gray-400 dark:hover:text-gray-300"
            >
