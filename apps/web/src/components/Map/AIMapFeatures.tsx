...
const AIMapFeatures: React.FC<AIMapFeaturesProps> = ({ 
  ...
  const analyzeMapData = useCallback(async () => {
    ...
    matchInsights.forEach(insight => newInsights.push(insight as any));

    // 5. Safety Analysis
    const safetyInsights = analyzeSafetyFactors(pins, userLocation);
    safetyInsights.forEach(insight => newInsights.push(insight as any));

    // 6. Activity Recommendations
    const activityRecs = generateActivityRecommendations(pins, userLocation);
    activityRecs.forEach(rec => newInsights.push(rec as any));
  ...
  if (!(user as any)?.premium?.isActive) {
    return (
