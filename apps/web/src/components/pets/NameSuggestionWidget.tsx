...
export function NameSuggestionWidget({
  petInfo,
  onNameSelect,
  className = '',
  showCategories = true,
  maxSuggestions = 12
}: NameSuggestionWidgetProps) {
  ...
  const getCategoryColor = (category: NameSuggestion['category']) => {
    const colors = {
  ...
  const getPopularityColor = (popularity: NameSuggestion['popularity']) => {
    const colors = {
    ...
    <div className={`bg-white dark:bg-gray-800 rounded-xl shadow-lg p-6 ${className}`}>
      ...
      {selectedName && (
        <motion.div
          initial={{ opacity: 0, y: 20 }}
          animate={{ opacity: 1, y: 0 }}
          className="mt-6 p-4 bg-gradient-to-r from-pink-50 to-purple-50 dark:from-pink-900/20 dark:to-purple-900/20 rounded-lg border border-pink-200 dark:border-pink-800"
        >
...
export function CompactNameSuggestion({ 
  petInfo, 
  onNameSelect,
  className = ''
}: {
      ...
      <AnimatePresence>
        {showSuggestions && (
          <motion.div
            ...
            className="absolute top-full left-0 mt-2 w-80 bg-white dark:bg-gray-800 rounded-lg shadow-lg border border-gray-200 dark:border-gray-700 p-4 z-50"
          >
            {isLoading ? (
...
function getCategoryColor(category: NameSuggestion['category']) {
  const colors = {
    classic: 'bg-blue-100 text-blue-800',
    trendy: 'bg-purple-100 text-purple-800',
    unique: 'bg-green-100 text-green-800',
