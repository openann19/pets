...
export default function GlobalTodoWidget({
  variant = 'compact',
  showStats = true,
  showCategories = true,
  maxHeight = '400px',
  className = '',
}: GlobalTodoWidgetProps) {
  const {
    todos,
    categories,
    stats,
  ...
  if (error) {
    return (
      <PremiumCard className={`p-4 border-red-500/50 ${className}`}>
  ...
  return (
    <PremiumCard className={`p-6 ${className}`} glow>
              ...
              >
                ...
                >
                  {todo.completed ? (
                    ...
                    <CircleIcon className="w-5 h-5 text-gray-400 hover:text-blue-400" />
                  )}
                </button>
