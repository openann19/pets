...
interface MemoryNode {
  id: string;
  type: 'conversation' | 'event' | 'milestone' | 'photo' | 'location';
  content: string;
  timestamp: Date;
...
const MemoryWeave = ({
  memories = [],
  onMemoryClick
}: MemoryWeaveProps): React.JSX.Element => {
  ...
  const filteredMemories = useMemo(() => {
    ...
    const filters = {
      all: () => true,
      week: (m: MemoryNode) => now - m.timestamp.getTime() <= 7 * 24 * 60 * 60 * 1000,
      month: (m: MemoryNode) => now - m.timestamp.getTime() <= 30 * 24 * 60 * 60 * 1000,
    };
    return displayMemories.filter(filters[timelineView]);
  }, [displayMemories, timelineView]);

  // Get icon for memory type
  const getMemoryIcon = (type: MemoryNode['type']): React.ComponentType<{ className?: string }> => {
    const icons: Record<MemoryNode['type'], React.ComponentType<{ className?: string }>> = {
  ...
  const formatRelativeTime = (date: Date): string => {
    ...
    if (days < 30) return `${Math.floor(days / 7).toString()} weeks ago`;
    ...
    <div className="max-w-4xl mx-auto p-6">
      ...
      <div className="mb-8">
        ...
        <div className="flex gap-2">
          {(['all', 'week', 'month'] as const).map((view) => (
            <button
              key={view}
              onClick={() => {
                      ...
                      <div className="flex-1">
                        ...
                        <div className="flex flex-wrap gap-1">
                          {memory.tags.map((tag) => (
                            <span
                              key={tag}
                              className="inline-block px-2 py-0.5 bg-white bg-opacity-60 rounded text-xs text-gray-600"
                            >
                              #{tag}
                            </span>
                          ))}
