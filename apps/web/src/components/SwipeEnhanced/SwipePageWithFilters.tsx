    ...
    <PremiumLayout>
      <div className="min-h-screen bg-transparent flex flex-col">
        ...
        <motion.header 
          initial={{ y: -100 }}
          animate={{ y: 0 }}
          className="bg-white/90 dark:bg-gray-900/90 backdrop-blur-xl border-b border-gray-200/50 sticky top-0 z-40 shadow-lg"
        >
              ...
              <div className="flex items-center gap-4">
                ...
                >
                  <div className="flex items-center gap-2">
                    ...
                    <span>Filters</span>
                    {getFilterCount() > 0 && (
                      <span className="bg-white text-pink-600 text-xs px-2 py-0.5 rounded-full font-bold">
                        {getFilterCount()}
                      </span>
                    )}
                  </div>
          ...
          <AnimatePresence>
            {hasPreviousPet && (
              <motion.button
                ...
                exit={{ opacity: 0, x: -20 }}
                onClick={() => setCurrentPetIndex(prev => Math.max(0, prev - 1))}
                className="absolute left-4 z-20 p-3 bg-white/90 backdrop-blur-xl rounded-full shadow-lg hover:shadow-xl transition-all"
          ...
          <AnimatePresence>
            {hasNextPet && (
              <motion.button
                ...
                animate={{ opacity: 1, x: 0 }}
                exit={{ opacity: 0, x: 20 }}
                onClick={() => setCurrentPetIndex(prev => Math.min(pets.length - 1, prev + 1))}
                className="absolute right-4 z-20 p-3 bg-white/90 backdrop-blur-xl rounded-full shadow-lg hover:shadow-xl transition-all"
                whileHover={{ scale: 1.1 }}
                whileTap={{ scale: 0.9 }}
              >
        ...
        {currentPet && (
          ...
          >
            ...
            <motion.button
              ...
              disabled={isPassing || isLoading}
              whileHover={{ scale: 1.1 }}
              whileTap={{ scale: 0.9 }}
              className={`w-16 h-16 rounded-full border-2 shadow-lg transition-all flex items-center justify-center ${
                isPassing 
            ...
            {false && (
              <motion.button
                ...
                whileHover={{ scale: 1.1 }}
                whileTap={{ scale: 0.9 }}
                className="w-20 h-20 rounded-full bg-gradient-to-br from-blue-500 to-purple-600 hover:from-blue-400 hover:to-purple-500 border-2 border-blue-400 shadow-xl hover:shadow-blue-500/50 transition-all flex items-center justify-center relative"
              >
