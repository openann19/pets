...
interface PremiumBadgeProps {
  ...
  variant?: 'default' | 'success' | 'warning' | 'error' | 'info' | 'premium' | 'holographic';
  size?: 'sm' | 'md' | 'lg';
  glow?: boolean;
  pulse?: boolean;
  className?: string;
...
export const StatusBadge = ({ status, ...props }: { status: 'online' | 'offline' | 'away' | 'busy' } & Omit<PremiumBadgeProps, 'children'>) => {
  const statusConfig = {
    online: { variant: 'success' as const, children: 'Online', icon: '🟢' },
    offline: { variant: 'default' as const, children: 'Offline', icon: '⚫' },
    away: { variant: 'warning' as const, children: 'Away', icon: '🟡' },
    busy: { variant: 'error' as const, children: 'Busy', icon: '🔴' },
  };

  const config = statusConfig[status];
  
  return (
    <PremiumBadge variant={config.variant} {...props}>
      {config.children}
    </PremiumBadge>
  );
};
