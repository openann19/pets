...
import type { ReportPayload } from '@pawfectmatch/core/types';
...
export function ReportDialog({ open, onOpenChange, targetId, category }: ReportDialogProps) {
    const [type, setType] = useState<ReportPayload['type']>('inappropriate_content');
    ...
    const submit = async () => {
        try {
            const payload: ReportPayload = { type, category, reason, description, targetId };
            ...
            <DialogContent>
                ...
                <div className="space-y-3">
                    <label className="block text-sm">Type
                        <select className="mt-1 w-full bg-white/10 border border-white/20 rounded p-2" value={type} onChange={(e) => setType(e.target.value as ReportPayload['type'])}>
