...
export const PhotoEditor = ({ image, onSave, onCancel }: PhotoEditorProps) => {
  ...
  const applySharpen = useCallback((imageData: ImageData, amount: number): ImageData => {
  ...
  const drawImage = useCallback(
    (img: HTMLImageElement, adj: Adjustments) => {
    ...
    <div className="fixed inset-0 z-50 bg-black flex flex-col">
      <div className="flex items-center justify-between p-4 bg-black/50 backdrop-blur-sm">
        ...
        <button
          ...
          className="flex items-center justify-center w-10 h-10 rounded-full bg-pink-500 hover:bg-pink-600 transition-colors disabled:opacity-50"
          aria-label="Save"
        >
          {isProcessing ? (
        ...
        <div className="flex border-b border-white/10">
          <button
            ...
            className={`flex-1 py-3 text-sm font-medium transition-colors ${
              activeTab === 'adjust' ? 'text-white border-b-2 border-pink-500' : 'text-white/60'
            }`}
          >
          ...
          <button
            ...
            className={`flex-1 py-3 text-sm font-medium transition-colors ${
              activeTab === 'filters' ? 'text-white border-b-2 border-pink-500' : 'text-white/60'
            }`}
          >
          ...
          {activeTab === 'adjust' ? (
            <div className="space-y-4">
              <AdjustmentSlider
                ...
                min={50}
                max={150}
                currentValue={adjustments.brightness}
              ...
              <AdjustmentSlider
                ...
                min={50}
                max={150}
                currentValue={adjustments.contrast}
              ...
              <AdjustmentSlider
                ...
                min={0}
                max={200}
                currentValue={adjustments.saturation}
              ...
              <AdjustmentSlider
                ...
                min={-50}
                max={50}
                currentValue={adjustments.warmth}
                onChange={(value) => setAdjustments((prev) => ({ ...prev, warmth: value }))}
              />
              <AdjustmentSlider
                label="Sharpen"
                ...
                min={0}
                max={100}
                currentValue={adjustments.sharpen}
                onChange={(value) => setAdjustments((prev) => ({ ...prev, sharpen: value }))}
              />
            ...
            <div className="grid grid-cols-3 gap-3">
              {[
                ...
                <button
                  ...
                  className="aspect-square rounded-lg bg-white/10 hover:bg-white/20 transition-colors flex items-center justify-center text-white text-sm font-medium"
                >
                  {filter.name}
                </button>
              ))}
...
const AdjustmentSlider = ({ label, value, min, max, currentValue, onChange }: AdjustmentSliderProps) => (
  <div>
