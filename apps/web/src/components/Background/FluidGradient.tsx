...
export default function FluidGradient() {
  ...
  const touchStateRef = useRef({
    ...
    touchPressure: 0,
    gestureType: 'none' as 'none' | 'tap' | 'drag' | 'pinch' | 'swipe',
    touchHistory: [] as Array<{ x: number; y: number; time: number }>,
  ...
  useEffect(() => {
    ...
    import('three').then((THREE) => {
      ...
      const calculateTouchVelocity = (touchState: typeof touchStateRef.current) => {
      ...
      // Detect gesture type
      const detectGesture = (touchState: typeof touchStateRef.current): 'none' | 'tap' | 'drag' | 'pinch' | 'swipe' => {
        ...
        if (duration < 200 && velocity < 0.1) return 'tap';
        if (velocity > 0.3) return 'swipe';
        if (duration > 100 && velocity > 0.05) return 'drag';
      ...
      const handleTouchStart = (event: TouchEvent) => {
        ...
        if (touchInfluence?.value !== undefined) touchInfluence.value = 1.0;
        if (touchPressure?.value !== undefined) touchPressure.value = 0.1;
      };
      ...
      const handleTouchMove = (event: TouchEvent) => {
        event.preventDefault();
        const touch = event.touches[0];
        if (!touch || !touchStateRef.current.isTouching) return;
      ...
      const animate = () => {
        ...
        if (smoothTouch?.value) {
          ...
          smoothTouch.value.x = smoothInterpolate(
            smoothTouch.value.x, 
            touchState.smoothTarget.x, 
            interpolationFactor
          );
          smoothTouch.value.y = smoothInterpolate(
            smoothTouch.value.y, 
            touchState.smoothTarget.y, 
            interpolationFactor
          );
        }
