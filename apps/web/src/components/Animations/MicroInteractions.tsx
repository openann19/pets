...
/**
 * Animated button with press, hover, and haptic feedback
 */
...
/**
 * Animated card with hover lift and tilt effect
 */
        ...
        >
            ...
            {hoverable && (
                <motion.div
                    ...
                    className="absolute -inset-1 bg-gradient-to-r from-purple-600 to-pink-600 rounded-xl blur-lg -z-10"
                />
            )}
...
export function AnimatedIcon({
    ...
    const getAnimation = () => {
        if (bounce) return { y: [0, -10, 0] };
        if (pulse) return { scale: [1, 1.2, 1] };
        if (spin) return { rotate: 360 };
        return {};
    };

    const getTransition = () => {
        if (bounce || pulse) return { duration: 0.6, repeat: Infinity, repeatDelay: 2 };
        if (spin) return { duration: 2, repeat: Infinity, ease: 'linear' };
        return {};
    };
...
export function SuccessCheckmark({ size = 'md', color = '#10b981', className = '' }: SuccessCheckmarkProps) {
    const sizeClasses = {
        sm: 'w-6 h-6',
        md: 'w-12 h-12',
        lg: 'w-16 h-16',
...
export function Badge({ children, pulse = false, variant = 'default', className = '', ...props }: BadgeProps) {
    const variants = {
...
export function NotificationDot({ count, pulse = true, className = '' }: NotificationDotProps) {
    return (
        <motion.div
            initial={{ scale: 0 }}
            animate={{ scale: 1 }}
...
export function Skeleton({ width = '100%', height = '20px', circle = false, className = '' }: SkeletonProps) {
    return (
        <motion.div
            initial={{ opacity: 0 }}
            animate={{ opacity: 1 }}
            className={`relative overflow-hidden bg-gray-200 ${circle ? 'rounded-full' : 'rounded-md'} ${className}`}
            style={{ width, height }}
        >
            <motion.div
                animate={{
        ...
        >
            ...
            {isVisible && (
                <motion.div
                    ...
                    className="absolute z-50 px-3 py-1.5 text-sm text-white bg-gray-900 rounded-lg whitespace-nowrap pointer-events-none"
