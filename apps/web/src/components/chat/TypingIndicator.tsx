  ...
  return (
    <AnimatePresence>
      <motion.div
        ...
        className={`flex items-center space-x-2 px-4 py-2 bg-gray-50 dark:bg-gray-800 rounded-lg ${className}`}
      >
        {/* Avatars */}
        {showAvatars && (
          <div className="flex -space-x-2">
            {typingUsers.slice(0, maxUsers).map((user, index) => (
              <motion.div
                key={user.userId}
            ...
            {typingUsers.length > maxUsers && (
              <div className="w-6 h-6 rounded-full border-2 border-white dark:border-gray-800 bg-gray-300 dark:bg-gray-600 flex items-center justify-center">
...
export function OnlineStatus({ userId, className = '', size = 'md' }: OnlineStatusProps) {
...
export function ChatInputWithTyping({
  chatId,
  onSendMessage,
  placeholder = "Type a message...",
  className = ""
}: ChatInputWithTypingProps) {
