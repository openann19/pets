...
export const SmartNotifications = ({
  ...
  const handleToggle = async (key: keyof NotificationPreferences, value: unknown) => {
  ...
  const handleQuietHoursTime = async (type: 'start' | 'end', time: string) => {
    const newPreferences = {
      ...preferences,
    ...
    <div className="max-w-2xl mx-auto space-y-6">
      ...
      <div className="bg-white dark:bg-gray-800 rounded-2xl p-6 shadow-lg border border-gray-200 dark:border-gray-700">
        ...
        <div className="space-y-4">
          <ToggleItem
            ...
            onChange={(checked) => handleToggle('matches', checked)}
          ...
          <ToggleItem
            ...
            onChange={(checked) => handleToggle('messages', checked)}
            disabled={isSaving || !preferences.enabled}
          />
          <ToggleItem
            label="Likes"
            ...
            onChange={(checked) => handleToggle('likes', checked)}
            disabled={isSaving || !preferences.enabled}
          />
          <ToggleItem
            label="Reminders"
            description="Daily check-in and activity reminders"
            icon="🔔"
      ...
      <div className="bg-white dark:bg-gray-800 rounded-2xl p-6 shadow-lg border border-gray-200 dark:border-gray-700">
        <div className="flex items-center justify-between mb-4">
          ...
          <button
            onClick={() => handleQuietHoursToggle(!preferences.quietHours.enabled)}
            disabled={isSaving}
            className={`relative inline-flex h-6 w-11 items-center rounded-full transition-colors ${preferences.quietHours.enabled ? 'bg-pink-600' : 'bg-gray-300 dark:bg-gray-600'
              }`}
          >
            <span
              className={`inline-block h-4 w-4 transform rounded-full bg-white transition-transform ${preferences.quietHours.enabled ? 'translate-x-6' : 'translate-x-1'
                }`}
            />
          </button>
        </div>
      ...
      <div className="bg-white dark:bg-gray-800 rounded-2xl p-6 shadow-lg border border-gray-200 dark:border-gray-700">
        ...
        <div className="space-y-3">
          {[
            ...
            <button
              key={option.value}
              onClick={() => handleToggle('frequency', option.value)}
              className={`w-full text-left p-4 rounded-xl border-2 transition-all ${preferences.frequency === option.value
      ...
      <div className="bg-white dark:bg-gray-800 rounded-2xl p-6 shadow-lg border border-gray-200 dark:border-gray-700">
        ...
        <div className="space-y-4">
          ...
          <ToggleItem
            ...
            onChange={(checked) => handleToggle('vibration', checked)}
            disabled={isSaving || !preferences.enabled}
          />
        </div>
      </div>
...
const ToggleItem = ({ label, description, icon, checked, onChange, disabled }: ToggleItemProps) => {
  return (
