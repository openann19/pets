  ...
  return (
    ...
    >
      ...
      <div className="relative z-10 flex items-center justify-center">
        {showInitials ? (
          <span className={`${sizeConfig.text} font-bold drop-shadow-sm`}>
            {initials}
          </span>
        ) : (
          <span className={`${sizeConfig.text} drop-shadow-sm`}>
            {petEmoji}
          </span>
        )}
      </div>
...
// Pet-specific avatar generator
interface PetAvatarProps extends Omit<AvatarGeneratorProps, 'name'> {
  petName: string
  species?: 'dog' | 'cat' | 'bird' | 'rabbit' | 'fish' | 'hamster' | 'turtle' | 'other'
  personality?: 'friendly' | 'energetic' | 'calm' | 'playful' | 'shy' | 'confident'
}
...
// User avatar generator with fallback
interface UserAvatarProps extends Omit<AvatarGeneratorProps, 'name'> {
  user: {
    name: string
    email?: string
    avatar?: string
...
export function UserAvatar({
  ...
  return (
    <AvatarGenerator
      ...
      size={size}
      className={className}
      showInitials={fallbackToInitials}
      {...props}
    />
  )
}

// Avatar group for multiple users/pets
