...
// Button-specific interactive wrapper
interface InteractiveButtonProps extends InteractiveProps {
...
// Card-specific interactive wrapper
interface InteractiveCardProps extends InteractiveProps {
...
// Swipe card specific wrapper
interface InteractiveSwipeCardProps extends InteractiveProps {
  onSwipeLeft?: () => void
  onSwipeRight?: () => void
  onSwipeUp?: () => void
}

export const InteractiveSwipeCard = forwardRef<HTMLDivElement, InteractiveSwipeCardProps>(({
  children,
  className,
  onSwipeLeft,
  onSwipeRight,
  ...
  return (
    <motion.div
      ...
      whileTap={{ scale: 0.98 }}
      transition={{ type: 'spring', stiffness: 300, damping: 20 }}
      className={clsx('transform-gpu will-change-transform cursor-grab active:cursor-grabbing', className)}
      {...motionProps}
    >
      {children}
    </motion.div>
...
// Modal/Dialog interactive wrapper
interface InteractiveModalProps extends InteractiveProps {
  isOpen: boolean
  onClose: () => void
}

export const InteractiveModal = forwardRef<HTMLDivElement, InteractiveModalProps>(({
  children,
  className,
  isOpen,
  onClose,
  ...
  return (
    ...
    >
      <motion.div
        ...
        className={clsx(
          'bg-white dark:bg-neutral-800 rounded-xl shadow-2xl max-w-md w-full',
          className
        )}
