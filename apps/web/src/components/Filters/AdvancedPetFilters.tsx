...
export const AdvancedPetFilters: React.FC<AdvancedPetFiltersProps> = ({ 
    ...
    const handleChange = useCallback((field: keyof PetFilters, val: any) => {
    ...
    return (
        <motion.form 
            ...
            initial="hidden"
            animate="visible"
            className="flex flex-col gap-6 p-6 bg-white dark:bg-gray-900 rounded-2xl shadow-xl border border-gray-200 dark:border-gray-700 backdrop-blur-lg max-w-md mx-auto"
            role="search"
            aria-label="Advanced pet filters"
        >
            {/* Header */}
            ...
            <motion.div variants={itemVariants} className="space-y-2">
                ...
                <select
                    ...
                    className="w-full px-3 py-2 rounded-lg border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-800 text-gray-900 dark:text-gray-100 focus:ring-2 focus:ring-pink-500 focus:border-transparent transition-all duration-200 hover:border-pink-300"
            ...
            <motion.div variants={itemVariants} className="grid grid-cols-2 gap-4">
                <div>
                    ...
                    <input
                        ...
                        className="w-full px-3 py-2 rounded-lg border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-800 text-gray-900 dark:text-gray-100 focus:ring-2 focus:ring-pink-500 focus:border-transparent transition-all duration-200"
                        aria-describedby="min-age-help"
                    />
                </div>
                <div>
                    <Label 
                    ...
                    {personalityTags.map((tag) => {
                        ...
                        return (
                            <motion.button
                                ...
                                whileTap={{ scale: 0.95 }}
                                className={`px-3 py-1.5 text-xs rounded-full border transition-all duration-200 focus:ring-2 focus:ring-pink-500 focus:outline-none ${
                                    isSelected
                                        ? 'bg-pink-500 text-white border-pink-500 shadow-md shadow-pink-500/30'
                                        : 'bg-white dark:bg-gray-800 text-gray-700 dark:text-gray-300 border-gray-300 dark:border-gray-600 hover:border-pink-300 hover:bg-pink-50 dark:hover:bg-gray-700'
                                }`}
                                aria-pressed={isSelected}
                                aria-label={`${isSelected ? 'Remove' : 'Add'} ${tag} personality tag`}
                            >
                                {tag}
                            </motion.button>
                        );
            ...
            <motion.div variants={itemVariants} className="flex gap-3 pt-4 border-t border-gray-200 dark:border-gray-700">
                {onReset && (
                    <Button
                        ...
                        variant="outline"
                        onClick={onReset}
                        className="flex-1 hover:bg-gray-50 dark:hover:bg-gray-800 transition-colors duration-200"
                ...
                {onApply && (
                    <Button
                        type="button"
                        onClick={onApply}
                        className="flex-1 bg-gradient-to-r from-pink-500 to-rose-500 hover:from-pink-600 hover:to-rose-600 text-white shadow-lg shadow-pink-500/30 transition-all duration-200 transform hover:scale-105"
