  ...
  const postCards = useMemo(() => {
    return posts.map((post) => (
      <PostCard
        key={post._id}
        post={post}
        showFullContent={selectedPost?._id === post._id}
        likeSubmitting={likeSubmitting[post._id]}
        commentSubmitting={false} // TODO: Pass actual comment submitting state
        followSubmitting={false} // TODO: Pass actual follow submitting state
        onLike={handleLike}
        onComment={() => setSelectedPost(post)}
        onShare={(postId) => onSharePost?.(postId)}
        onJoinActivity={(activityId) => onJoinActivity?.(activityId)}
