...
import type { PackGroup, User } from '@pawfectmatch/core';
import {
  Clock,
  Heart,
  MapPin,
...
interface AICommunitySuggestionsProps {
  userId: string;
  userProfile: {
    location: string;
    interests: string[];
  ...
  onConnectWithUser?: (userId: string) => void;
  onViewProfile?: (userId: string) => void;
}

export const AICommunitySuggestions = memo(
  ({
    userId,
    userProfile,
    onJoinPack,
    onConnectWithUser,
    onViewProfile,
  }: AICommunitySuggestionsProps) => {
    const [suggestions, setSuggestions] = useState<{
      packGroups: Array<PackGroup & { compatibilityScore: number; reasons: string[] }>;
      users: Array<
        User & { compatibilityScore: number; reasons: string[]; commonInterests: string[] }
      >;
      activities: Array<{
        _id: string;
        title: string;
    ...
    const getCompatibilityBadge = useCallback((score: number) => {
      if (score >= 90) return { text: 'Perfect Match', variant: 'default' as const };
      if (score >= 80) return { text: 'Great Match', variant: 'secondary' as const };
      if (score >= 70) return { text: 'Good Match', variant: 'outline' as const };
      return { text: 'Worth Considering', variant: 'outline' as const };
    }, []);

    const generateMockSuggestions = useCallback(
      (profile: typeof userProfileAnalysis) => {
                ...
                return (
                  ...
                  >
                    ...
                    <div className="flex gap-2">
                      <Button
                        onClick={() => onConnectWithUser?.(user._id)}
                        className="flex-1"
                      >
