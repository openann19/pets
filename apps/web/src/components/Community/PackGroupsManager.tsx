...
interface PackGroupsManagerProps {
  userId?: string;
}

const PackGroupsManager = ({ }: PackGroupsManagerProps): React.JSX.Element => {
  const [searchQuery, setSearchQuery] = useState('');
  const [selectedCategory, setSelectedCategory] = useState<'all' | 'nearby' | 'joined'>('all');
  const [showCreateModal, setShowCreateModal] = useState(false);

  // Sample pack groups data
  const [groups] = useState<PackGroup[]>([
    {
      id: '1',
      ...
      <div className="mb-8">
        ...
        <div className="space-y-4">
          ...
          <div className="flex gap-2">
            {(['all', 'nearby', 'joined'] as const).map((category) => (
              <button
                key={category}
