 ...
 */
const DashboardBackdrop: React.FC<{ className?: string }> = ({ className = '' }) => {
  // const { data: weather } = useWeather();
  const weather = null; // Stub for now

  const gradientClass = useMemo(() => {
    if (!weather) return WEATHER_GRADIENTS.Default;
    const key = (weather as any).weather[0].main as keyof typeof WEATHER_GRADIENTS;
  ...
  return (
    <motion.div
      ...
      className={`fixed inset-0 -z-10 bg-gradient-to-br ${gradientClass} ${className}`}
      initial={{ opacity: 0 }}
      animate={{ opacity: 1 }}
