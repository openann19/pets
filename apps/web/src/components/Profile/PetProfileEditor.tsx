...
export interface PetProfile {
  ...
  socialization?: 'shy' | 'neutral' | 'friendly' | 'very-friendly';
  training?: string[];
}

interface PetProfileEditorProps {
  pet: Partial<PetProfile>;
  onSave: (updatedPet: Partial<PetProfile>) => Promise<void>;
  isLoading?: boolean;
}

export const PetProfileEditor = ({ pet, onSave, isLoading = false }: PetProfileEditorProps) => {
  const [profile, setProfile] = useState<Partial<PetProfile>>(pet);
  const [showAvatarCropper, setShowAvatarCropper] = useState(false);
  const [showCoverCropper, setShowCoverCropper] = useState(false);
  const [selectedImage, setSelectedImage] = useState('');
  const [activeTab, setActiveTab] = useState<'basic' | 'personality' | 'medical' | 'preferences'>(
    'basic',
  );
  const [showTooltip, setShowTooltip] = useState<string | null>(null);

  const handleChange = (field: keyof PetProfile, value: unknown) => {
    setProfile({ ...profile, [field]: value });
  };

  const handleStringArrayChange = (field: string, value: string) => {
    // Handle string arrays like personality traits, likes, dislikes
    const currentArray = (profile[field as keyof PetProfile] as string[]) || [];
  ...
  const handleRemoveStringArrayItem = (field: string, index: number) => {
    const currentArray = (profile[field as keyof PetProfile] as string[]) || [];
    setProfile({
      ...profile,
  ...
  const handleCropComplete = async (blob: Blob, type: 'avatar' | 'cover') => {
  ...
  const Tooltip = ({ id, text }: { id: string; text: string }) => (
    <div
      className="absolute z-10 bg-gray-900 text-white p-2 rounded-lg text-xs w-48"
      style={{
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-2">
          {speciesOptions.map((option) => (
            <button
              key={option.value}
              type="button"
              className={`p-2 rounded-lg border text-center transition-colors ${profile.species === option.value
        ...
        <div className="grid grid-cols-3 gap-2">
          {genderOptions.map((option) => (
            <button
              key={option.value}
              type="button"
              className={`p-3 rounded-lg border text-center transition-colors ${profile.gender === option.value
                  ? 'bg-pink-100 dark:bg-pink-900/30 border-pink-500 text-pink-800 dark:text-pink-300'
                  : 'bg-white dark:bg-gray-800 border-gray-300 dark:border-gray-600 text-gray-700 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-gray-700'
                }`}
      ...
      <div>
        <div className="flex items-center justify-between mb-2">
          ...
          <div className="relative">
            <button
              ...
              onMouseEnter={() => setShowTooltip('personality')}
            ...
            <Tooltip
              id="personality"
              text="Add traits like Playful, Calm, Energetic, Curious, Shy, etc."
            />
          </div>
        </div>
        <div className="flex flex-wrap gap-2 mb-2">
          {(profile.personality || []).map((trait, index) => (
            <span
              key={index}
              className="px-3 py-1 bg-pink-100 dark:bg-pink-900/30 text-pink-800 dark:text-pink-300 rounded-full text-sm flex items-center"
            >
              {trait}
              <button
                type="button"
                onClick={() => handleRemoveStringArrayItem('personality', index)}
                className="ml-1 text-pink-600 hover:text-pink-800 dark:hover:text-pink-400"
              >
                ×
              </button>
            </span>
        ...
        <div className="flex space-x-2">
          <input
            ...
            onKeyPress={(e) => {
              if (e.key === 'Enter') {
                e.preventDefault();
                handleStringArrayChange('personality', (e.target as HTMLInputElement).value);
          ...
          <button
            ...
            onClick={() => {
              const input = document.getElementById('personality') as HTMLInputElement;
        ...
        <div className="space-y-2">
          {activityOptions.map((option) => (
            <button
              key={option.value}
              type="button"
              className={`w-full p-3 rounded-lg border text-left transition-colors flex items-center ${profile.activityLevel === option.value
                  ? 'bg-pink-100 dark:bg-pink-900/30 border-pink-500 text-pink-800 dark:text-pink-300'
                  : 'bg-white dark:bg-gray-800 border-gray-300 dark:border-gray-600 text-gray-700 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-gray-700'
                }`}
        ...
        <div className="space-y-2">
          {socializationOptions.map((option) => (
            <button
              key={option.value}
              type="button"
              className={`w-full p-3 rounded-lg border text-left transition-colors flex items-center ${profile.socialization === option.value
                  ? 'bg-pink-100 dark:bg-pink-900/30 border-pink-500 text-pink-800 dark:text-pink-300'
                  : 'bg-white dark:bg-gray-800 border-gray-300 dark:border-gray-600 text-gray-700 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-gray-700'
                }`}
              onClick={() => handleChange('socialization', option.value)}
            >
              <span className="w-4 h-4 mr-2 rounded-full border border-current inline-block"></span>
              {option.label}
            </button>
    ...
    <div className="bg-white dark:bg-gray-800 rounded-xl shadow-lg border border-gray-200 dark:border-gray-700 overflow-hidden">
      ...
      >
        ...
        <div className="flex border-b border-gray-200 dark:border-gray-700 mb-6 overflow-x-auto scrollbar-thin scrollbar-thumb-gray-300 dark:scrollbar-thumb-gray-600">
          <button
            type="button"
            className={`py-2 px-4 font-medium text-sm border-b-2 whitespace-nowrap ${activeTab === 'basic'
          ...
          <button
            type="button"
            className={`py-2 px-4 font-medium text-sm border-b-2 whitespace-nowrap ${activeTab === 'personality'
                ? 'border-pink-500 text-pink-600 dark:text-pink-400'
                : 'border-transparent text-gray-500 hover:text-gray-700 dark:text-gray-400 dark:hover:text-gray-300'
              }`}
          ...
          <button
            type="button"
            className={`py-2 px-4 font-medium text-sm border-b-2 whitespace-nowrap ${activeTab === 'medical'
                ? 'border-pink-500 text-pink-600 dark:text-pink-400'
                : 'border-transparent text-gray-500 hover:text-gray-700 dark:text-gray-400 dark:hover:text-gray-300'
              }`}
          ...
          <button
            type="button"
            className={`py-2 px-4 font-medium text-sm border-b-2 whitespace-nowrap ${activeTab === 'preferences'
                ? 'border-pink-500 text-pink-600 dark:text-pink-400'
                : 'border-transparent text-gray-500 hover:text-gray-700 dark:text-gray-400 dark:hover:text-gray-300'
              }`}
        ...
        <div className="flex justify-end space-x-3">
          <button
            type="submit"
            disabled={isLoading}
            className="py-2 px-6 bg-pink-600 hover:bg-pink-700 text-white font-medium rounded-lg transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
          >
            {isLoading ? (
