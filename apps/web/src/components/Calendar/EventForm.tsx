...
export const EventForm = ({ date, pets, onSave, onCancel, event }: EventFormProps) => {
  ...
  const [eventType, setEventType] = useState<CalendarEvent['type']>(event?.type || 'playdate');
  const [selectedPets, setSelectedPets] = useState<string[]>(event?.petIds || []);
    ...
    <div className="bg-white dark:bg-gray-800 rounded-xl shadow-lg border border-gray-200 dark:border-gray-700 p-5">
      ...
      >
          ...
          <div className="grid grid-cols-2 md:grid-cols-5 gap-2">
            {eventTypes.map((type) => (
              <button
                key={type.value}
                type="button"
                className={`p-2 rounded-lg border border-transparent text-center text-sm transition-colors ${
                  eventType === type.value
                    ? `${type.color} text-white border-white`
                    : 'bg-gray-100 dark:bg-gray-700 text-gray-700 dark:text-gray-300 hover:bg-gray-200 dark:hover:bg-gray-600'
                }`}
                onClick={() => setEventType(type.value as CalendarEvent['type'])}
              >
                {type.label}
              </button>
            ))}
          ...
          <div className="flex items-center mb-2">
            ...
            <button
              ...
              className={`ml-2 relative inline-flex h-6 w-11 items-center rounded-full transition-colors ${
                allDay ? 'bg-pink-600' : 'bg-gray-300 dark:bg-gray-600'
              }`}
        ...
        <div className="flex space-x-3">
          <button
            type="submit"
            className="flex-1 py-2 px-4 bg-pink-600 hover:bg-pink-700 text-white font-medium rounded-lg transition-colors"
          >
