...
type CalendarEvent = {
  ...
  type: 'playdate' | 'vet' | 'grooming' | 'training' | 'other';
  petIds: string[];
  location?: string;
  color?: string;
};
  ...
  const renderDays = () => {
    ...
    for (let i = 0; i < 7; i++) {
      days.push(
        <div
          key={i}
          className="font-semibold text-center text-sm text-gray-600 dark:text-gray-400 py-2"
        >
        ...
        <div
          ...
          className={`min-h-[100px] p-2 border border-gray-200 dark:border-gray-700 ${
            ...
            isSameDay(day, new Date())
              ...
              : ''
          } ${isSameDay(day, activeDate) ? 'ring-2 ring-pink-500 dark:ring-pink-600' : ''}`}
          onClick={() => {
        ...
        >
          ...
          <div className="space-y-1 overflow-y-auto max-h-[80px] scrollbar-thin scrollbar-thumb-gray-300 dark:scrollbar-thumb-gray-700">
            {dayEvents.slice(0, 3).map((event) => (
              <div
                key={event.id}
                className={`text-xs p-1 rounded truncate text-white ${getEventColors(event.type)} cursor-pointer`}
                onClick={(e) => {
                  e.stopPropagation();
                  onEventClick?.(event);
                }}
        ...
        <div className="border-t border-gray-200 dark:border-gray-700 p-4 bg-gray-50 dark:bg-gray-900/50">
          ...
          <div className="space-y-2">
            {todaysEvents.map((event) => (
              <div
                key={event.id}
                className="flex items-center space-x-3 p-2 bg-white dark:bg-gray-800 rounded-lg border border-gray-200 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-700/50 cursor-pointer transition-colors"
                onClick={() => onEventClick?.(event)}
              >
                <div
                  className={`w-10 h-10 rounded-full flex items-center justify-center text-white ${getEventColors(event.type)}`}
                >
