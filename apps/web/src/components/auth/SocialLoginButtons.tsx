    ...
    <div className={`space-y-4 ${className}`}>
      ...
      >
        <InteractiveButton
          ...
          icon={
            isLoading === 'google' ? (
              <div className={`${iconSize[size]} animate-spin rounded-full border-2 border-gray-300 border-t-gray-600`} />
            ) : (
              <GoogleIcon className={iconSize[size]} />
            )
          }
        >
          {showLabels && (
      ...
      >
        <InteractiveButton
          ...
          icon={
            isLoading === 'apple' ? (
              <div className={`${iconSize[size]} animate-spin rounded-full border-2 border-white border-t-transparent`} />
            ) : (
              <AppleIcon className={iconSize[size]} />
            )
          }
        >
          {showLabels && (
      ...
      {error && (
        <motion.div
          ...
          className="bg-red-50 border-2 border-red-200 text-red-800 p-4 rounded-xl"
        >
  ...
  return (
    <div className="flex space-x-3">
      <InteractiveButton
        ...
        className="flex-1 bg-white hover:bg-gray-50 text-gray-900 border border-gray-300 hover:border-gray-400 p-3"
        icon={<GoogleIcon className="w-5 h-5" />}
      />
      <InteractiveButton
        onClick={() => signIn('apple', { callbackUrl: '/dashboard' })}
        className="flex-1 bg-black hover:bg-gray-900 text-white border border-black hover:border-gray-800 p-3"
