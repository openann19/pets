...
export const TwoFactorAuth = ({ onVerify, onSetup, mode, onCancel }: TwoFactorAuthProps) => {
  const [code, setCode] = useState(['', '', '', '', '', '']);
  const [isVerifying, setIsVerifying] = useState(false);
  const [error, setError] = useState('');
  const [qrCode, setQrCode] = useState('');
  const [secret, setSecret] = useState('');
  const [step, setStep] = useState<'qr' | 'verify'>(mode === 'setup' ? 'qr' : 'verify');
              ...
              <input
                ...
                className={`w-12 h-14 text-center text-2xl font-bold border-2 rounded-xl transition-all ${
                  error
                    ...
                    : 'border-gray-300 dark:border-gray-600 focus:border-pink-500 dark:focus:border-pink-500'
                } bg-white dark:bg-gray-800 text-gray-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-pink-500/20`}
