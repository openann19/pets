...
export function EnhancementProvider({
  ...
  return (
    <div className="relative">
      ...
      {showTourLauncher && isAuthenticated && (
        <TourLauncher 
      ...
      {/* Offline Indicator */}
      {isOffline && (
        <div className="fixed top-4 left-4 z-50 bg-yellow-500 text-white px-4 py-2 rounded-lg shadow-lg">
      ...
      {showPushNotifications && isAuthenticated && (
      ...
      {showGamification && isAuthenticated && user && (
...
export function withEnhancements<P extends object>(
  Component: React.ComponentType<P>,
  options: Partial<EnhancementProviderProps> = {}
) {
  return function EnhancedComponent(props: P) {
    return (
      <EnhancementProvider {...options}>
        <Component {...props} />
      </EnhancementProvider>
    )
  }
...
export function useEnhancements() {
  ...
  const trackMatch = async (matchId: string, petId: string) => {
    // Track for gamification
    recordActivity({
      type: 'match',
  ...
  const trackChatMessage = async (chatId: string, message: string) => {
    // Track for gamification
    recordActivity({
      type: 'chat',
