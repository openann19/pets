...
export default function AdvancedFilterPanel({
  ...
  const [activeTab, setActiveTab] = useState<'species' | 'characteristics' | 'preferences' | 'advanced'>('species');
  ...
  const getActiveTabCount = (tab: string) => {
    switch (tab) {
      case 'species':
      ...
      {isOpen && (
        ...
        >
          ...
          <motion.div
            ...
            className={`relative bg-white dark:bg-gray-900 rounded-3xl shadow-2xl border border-gray-200/50 dark:border-gray-700/50 max-w-4xl w-full max-h-[90vh] overflow-hidden ${className}`}
          >
            {/* Header */}
                ...
                {activeTab === 'species' && (
                  ...
                  >
                    ...
                    <div>
                      ...
                      {/* Selected Breeds */}
                      {localFilters.breeds.length > 0 && (
                        <div className="mt-4">
                          <div className="flex flex-wrap gap-2">
                            {localFilters.breeds.map((breed, index) => (
                              <span
                                key={index}
                                className="inline-flex items-center gap-1 px-3 py-1.5 bg-gradient-to-r from-pink-100 to-purple-100 dark:from-pink-900/20 dark:to-purple-900/20 text-pink-800 dark:text-pink-200 rounded-full text-sm border border-pink-200 dark:border-pink-800"
                              >
                                {breed}
                                <button
                                  onClick={() => {
                    ...
                    <div>
                      ...
                      <div className="grid grid-cols-4 gap-3">
                        {intentOptions.map((intent) => (
                          <button
                            key={intent.value}
                            onClick={() => updateFilter('intent', [intent.value])}
                            className={`p-4 rounded-xl border-2 transition-all text-center ${
                              localFilters.intent.includes(intent.value)
                ...
                {activeTab === 'characteristics' && (
                  ...
                  >
                    ...
                    <div>
                      ...
                      <div className="flex items-center gap-4">
                        <input
                          ...
                          onChange={(e) => updateFilter('ageRange', [parseInt(e.target.value) || 0, localFilters.ageRange[1]])}
                        ...
                        <input
                          ...
                          onChange={(e) => updateFilter('ageRange', [localFilters.ageRange[0], parseInt(e.target.value) || 20])}
                          className="w-20 px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-800 text-gray-900 dark:text-white"
                        />
                      </div>
                    </div>
                    ...
                    <div>
                      ...
                      <div className="grid grid-cols-2 gap-3">
                        <button
                          onClick={() => toggleArrayFilter('gender', 'male')}
                          className={`p-4 rounded-xl border-2 transition-all ${
                        ...
                        <button
                          onClick={() => toggleArrayFilter('gender', 'female')}
                          className={`p-4 rounded-xl border-2 transition-all ${
                      ...
                      <div className="grid grid-cols-2 gap-3">
                        {energyLevelOptions.map((energy) => (
                          <button
                            ...
                            className={`p-4 rounded-xl border-2 transition-all ${
                              localFilters.energyLevels.includes(energy.value)
                    ...
                    <div>
                      ...
                      <div className="grid grid-cols-3 gap-2">
                        {temperamentOptions.map((temperament) => (
                          <button
                            key={temperament}
                            onClick={() => toggleArrayFilter('temperaments', temperament)}
                            className={`px-3 py-2 rounded-lg text-sm font-medium transition-all ${
                              localFilters.temperaments.includes(temperament)
                          ...
                          >
                            {temperament.replace('-', ' ').split(' ').map(word => 
                    ...
                    <div>
                      ...
                      <div className="space-y-3">
                        <label className="flex items-center space-x-3 p-3 rounded-xl border border-gray-200 dark:border-gray-700 hover:border-pink-300 dark:hover:border-pink-600 transition-colors">
                          <input
                            type="checkbox"
                            checked={localFilters.apartmentFriendly === true}
                            onChange={(e) => updateFilter('apartmentFriendly', e.target.checked ? true : null)}
            ...
            <div className="flex items-center justify-between p-6 border-t border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-800/50">
              ...
              <div className="flex gap-3">
                ...
                <button
                  onClick={handleApply}
                  className="px-6 py-2 bg-gradient-to-r from-pink-500 to-purple-600 text-white rounded-xl hover:from-pink-600 hover:to-purple-700 transition-all shadow-lg hover:shadow-xl"
                >
                  Apply Filters
                </button>
              </div>
