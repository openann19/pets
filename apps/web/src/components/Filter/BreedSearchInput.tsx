...
interface BreedSearchInputProps {
  value: string;
  onChange: (value: string, breed?: Breed) => void;
  onSelect?: (breed: Breed) => void;
  species?: string;
  placeholder?: string;
  className?: string;
  showPopularity?: boolean;
  showPetCount?: boolean;
...
export default function BreedSearchInput({
  ...
  onSelect,
  species,
  placeholder = "Search breeds (e.g., 'shiba inu')",
  className = '',
  showPopularity = true,
  showPetCount = true,
  variant = 'elegant'
  ...
  return (
    <div className={`relative ${className}`}>
      {/* Search Input */}
      <motion.div
        initial={variant === 'elegant' ? { opacity: 0, scale: 0.95 } : false}
        animate={{ opacity: 1, scale: 1 }}
        transition={{ duration: 0.2 }}
        className={`relative ${styles.input}`}
      >
        <div className="flex items-center px-4 py-3">
          ...
          <input
            ...
            className="flex-1 bg-transparent outline-none placeholder-gray-400 dark:placeholder-gray-500 text-gray-900 dark:text-white"
          />
          ...
          {isLoading && (
            <motion.div
              ...
              className="w-5 h-5 border-2 border-pink-500 border-t-transparent rounded-full ml-3"
            />
          )}
        </div>
      </motion.div>
      ...
      <AnimatePresence>
        {isOpen && suggestions.length > 0 && (
          <motion.div
            ...
            transition={{ duration: 0.2 }}
            className={`absolute top-full left-0 right-0 mt-2 z-50 max-h-80 overflow-y-auto ${styles.suggestions}`}
          >
            <div className="p-2">
                  ...
                  <div className="flex items-center justify-between">
                    <div className="flex-1">
                      ...
                      <div className="flex items-center gap-2 text-sm text-gray-600 dark:text-gray-400">
                        ...
                        {showPetCount && breed.availablePets !== undefined && (
                    ...
                    {showPopularity && breed.popularity > 0 && (
