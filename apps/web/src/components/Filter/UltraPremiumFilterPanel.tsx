...
interface UltraPremiumFilterPanelProps {
  isOpen: boolean;
  onClose: () => void;
  onApply: (filters: UltraFilterState) => void;
  defaultFilters?: Partial<UltraFilterState>;
  className?: string;
}
...
export default function UltraPremiumFilterPanel({
  isOpen,
  onClose,
  onApply,
  defaultFilters = {},
  className = ''
}: UltraPremiumFilterPanelProps) {
  ...
  const updateFilter = <K extends keyof UltraFilterState>(key: K, value: UltraFilterState[K]) => {
    setFilters(prev => ({ ...prev, [key]: value }));
  };

  const toggleArrayFilter = <K extends keyof UltraFilterState>(key: K, value: any) => {
    const currentArray = Array.isArray(filters[key]) ? filters[key] as any[] : [];
    const newArray = currentArray.includes(value)
      ? currentArray.filter(item => item !== value)
      : [...currentArray, value];
    updateFilter(key, newArray as UltraFilterState[K]);
  };
      ...
      {isOpen && (
        ...
        >
          ...
          <motion.div
            ...
            className={`relative bg-white dark:bg-gray-900 rounded-3xl shadow-2xl border border-gray-200/50 dark:border-gray-700/50 max-w-6xl w-full max-h-[95vh] overflow-hidden ${className}`}
          >
                      ...
                      <div>
                        ...
                        <div className="grid grid-cols-2 lg:grid-cols-3 gap-4">
                          {speciesOptions.map((species) => (
                            <motion.button
                              key={species.value}
                              onClick={() => toggleArrayFilter('species', species.value)}
                              whileHover={{ scale: 1.02 }}
                              whileTap={{ scale: 0.98 }}
                              className={`p-6 rounded-2xl border-2 transition-all duration-300 ${
                                filters.species.includes(species.value)
                      ...
                      {filters.species.length > 0 && (
                        <div>
                          ...
                          <BreedSearchInput
                            ...
                            placeholder="Search breeds (e.g., 'golden retriever', 'persian cat')"
                            variant="elegant"
                            showPopularity={true}
                            showPetCount={true}
                            className="w-full"
                          ...
                          {filters.breeds.length > 0 && (
                            <div className="mt-6">
                              ...
                              <div className="flex flex-wrap gap-3">
                                {filters.breeds.map((breed, index) => (
                                  <motion.span
                                    key={index}
                                    initial={{ opacity: 0, scale: 0.8 }}
                                    animate={{ opacity: 1, scale: 1 }}
                                    className="inline-flex items-center gap-2 px-4 py-2 bg-gradient-to-r from-pink-100 to-purple-100 dark:from-pink-900/20 dark:to-purple-900/20 text-pink-800 dark:text-pink-200 rounded-full text-sm font-medium border border-pink-200 dark:border-pink-800 shadow-sm"
                                  >
                                    <span>{breed}</span>
                                    <button
                                      onClick={() => updateFilter('breeds', filters.breeds.filter((_, i) => i !== index))}
                      ...
                      <div>
                        ...
                        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-3">
                          {temperamentOptions.map((trait) => (
                            <motion.button
                              key={trait.value}
                              onClick={() => toggleArrayFilter('temperaments', trait.value)}
                              whileHover={{ scale: 1.05 }}
                              whileTap={{ scale: 0.95 }}
                              className={`p-4 rounded-xl border-2 transition-all duration-200 ${
                                filters.temperaments.includes(trait.value)
                      ...
                      <div>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                          {energyLevelOptions.map((energy) => (
                            <motion.button
                              key={energy.value}
                              onClick={() => toggleArrayFilter('energyLevels', energy.value)}
                              whileHover={{ scale: 1.02 }}
                              whileTap={{ scale: 0.98 }}
                              className={`p-6 rounded-xl border-2 transition-all duration-200 text-left ${
                                filters.energyLevels.includes(energy.value)
                    ...
                    >
                      ...
                      <div>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                          {Object.entries(filters.premiumFeatures).map(([key, value]) => (
                            <motion.div
                              key={key}
                              whileHover={{ scale: 1.02 }}
                              className={`p-6 rounded-2xl border-2 transition-all duration-300 ${
                            ...
                            >
                              <label className="flex items-center gap-4 cursor-pointer">
                                <div className={`w-12 h-12 rounded-xl flex items-center justify-center ${
                                  value ? 'bg-gradient-to-r from-yellow-400 to-orange-500' : 'bg-gray-100 dark:bg-gray-800'
                                }`}>
                                ...
                                <input
                                  type="checkbox"
                                  checked={value}
                                  onChange={(e) => updateFilter('premiumFeatures', {
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                          <div>
                            ...
                            <select
                              ...
                              className="w-full p-3 border border-gray-300 dark:border-gray-600 rounded-xl bg-white dark:bg-gray-800 text-gray-900 dark:text-white focus:ring-2 focus:ring-pink-500 focus:border-transparent"
                            >
                          ...
                          <div>
                            ...
                            <select
                              ...
                              onChange={(e) => updateFilter('resultLimit', parseInt(e.target.value))}
                              className="w-full p-3 border border-gray-300 dark:border-gray-600 rounded-xl bg-white dark:bg-gray-800 text-gray-900 dark:text-white focus:ring-2 focus:ring-pink-500 focus:border-transparent"
                            >
            ...
            <div className="flex items-center justify-between p-6 bg-gradient-to-r from-gray-50 to-white dark:from-gray-800/50 dark:to-gray-900/50 border-t border-gray-200 dark:border-gray-700">
              ...
              <div className="flex gap-3">
                <motion.button
                  ...
                  whileHover={{ scale: 1.05 }}
                  whileTap={{ scale: 0.95 }}
                  className="px-6 py-2 border border-gray-300 dark:border-gray-600 text-gray-700 dark:text-gray-300 rounded-xl hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors font-medium"
                >
                  Cancel
                </motion.button>
                ...
                <motion.button
                  ...
                  whileHover={{ scale: 1.05 }}
                  whileTap={{ scale: 0.95 }}
                  className="px-8 py-2 bg-gradient-to-r from-pink-500 to-purple-600 text-white rounded-xl hover:from-pink-600 hover:to-purple-700 transition-all shadow-lg hover:shadow-xl font-medium flex items-center gap-2"
                >
