...
export const TravelMode = ({ isActive, onToggle, onLocationChange }: TravelModeProps) => {
    ...
    <div className="bg-white dark:bg-gray-800 rounded-lg p-6 shadow-lg">
      <div className="flex items-center justify-between mb-6">
        <div className="flex items-center space-x-3">
          <div
            className={`p-2 rounded-full ${isActive ? 'bg-green-100 dark:bg-green-900' : 'bg-gray-100 dark:bg-gray-700'}`}
          >
            <GlobeAltIcon
        ...
        <button
          ...
          className={`relative inline-flex h-6 w-11 items-center rounded-full transition-colors ${
            isActive ? 'bg-green-600' : 'bg-gray-200 dark:bg-gray-600'
          }`}
        >
          <span
              ...
              {suggestions.map((location) => (
                ...
                >
                  <div className="flex items-center justify-between">
                    ...
                    {isLoading && selectedLocation?.id === location.id && (
                      <ArrowPathIcon className="h-4 w-4 text-pink-600 animate-spin" />
                    )}
                  </div>
                </motion.button>
              ))}
