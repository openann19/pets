...
type LocationSettings = {
  ...
  shareWithPremium: boolean;
  locationUpdateFrequency: 'realtime' | 'hourly' | 'daily' | 'manual';
};
...
export const LocationPrivacy = ({ currentSettings, onSettingsChange }: LocationPrivacyProps) => {
    ...
    <div className="bg-white dark:bg-gray-800 rounded-lg p-6 shadow-lg">
            ...
            >
              <div className="flex items-center justify-between">
                ...
                <button
                  ...
                  className={`relative inline-flex h-6 w-11 items-center rounded-full transition-colors ${isEnabled ? 'bg-green-600' : 'bg-gray-200 dark:bg-gray-600'
                    }`}
                >
                  <span
          ...
          {updateFrequencyOptions.map((option) => (
            ...
            >
              <input
                ...
                value={option.value}
                checked={settings.locationUpdateFrequency === option.value}
                onChange={(e) => handleSettingChange('locationUpdateFrequency', e.target.value)}
      ...
      <div className="bg-gray-50 dark:bg-gray-700 rounded-lg p-4">
        ...
        <div className="space-y-2 text-sm text-gray-600 dark:text-gray-400">
          ...
          {settings.showCity && !settings.showExactLocation && (
            <div className="flex items-center space-x-2">
              <EyeIcon className="h-4 w-4 text-yellow-500" />
              <span>City is visible to others</span>
            </div>
          )}
          {settings.showCountry && !settings.showCity && !settings.showExactLocation && (
            <div className="flex items-center space-x-2">
              <EyeIcon className="h-4 w-4 text-green-500" />
              <span>Country is visible to others</span>
            </div>
          )}
          {!settings.showExactLocation && !settings.showCity && !settings.showCountry && (
            <div className="flex items-center space-x-2">
