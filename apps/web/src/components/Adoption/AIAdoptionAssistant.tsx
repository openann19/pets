...
import type { AdoptionApplication, Pet, CompatibilityAnalysis } from '@pawfectmatch/core';

interface AIAdoptionAssistantProps {
  application: AdoptionApplication;
  pet: Pet;
  onCompatibilityUpdate?: (analysis: CompatibilityAnalysis) => void;
  onRecommendationAction?: (
    action: 'approve' | 'reject' | 'schedule_meetup' | 'request_more_info',
  ) => void;
}

export const AIAdoptionAssistant = ({
  application,
  pet,
  onCompatibilityUpdate,
  onRecommendationAction,
}: AIAdoptionAssistantProps) => {
  ...
  const analyzeCompatibility = async () => {
    ...
    setRiskFactors(
      mockAnalysis.factors.filter(
        (factor) => factor.includes('risk') || factor.includes('concern'),
      ),
    );
  ...
  const generateMockAnalysis = (app: AdoptionApplication, pet: Pet): CompatibilityAnalysis => {
    ...
    if (app.petExperience.hasOwnedPets) score += 5;
    if (app.livingSituation.residenceType === 'house') score += 3;
    if (app.livingSituation.yardType === 'fenced') score += 2;
    if (app.lifestyle.budget.emergencyFund) score += 3;
    if (app.references.length >= 3) score += 2;

    // Species preference match
    if (app.petExperience.petPreferences.species.includes(pet.species)) score += 5;

    // Size preference match
    if (app.petExperience.petPreferences.size.includes(pet.size)) score += 3;

    // Age range match
    const ageInRange =
      pet.age >= app.petExperience.petPreferences.ageRange.min &&
      pet.age <= app.petExperience.petPreferences.ageRange.max;
    if (ageInRange) score += 2;
    ...
    if (
      app.livingSituation.residenceType === 'apartment' &&
        ...
        <CardContent>
          <div className="space-y-4">
            ...
            <div className="flex flex-wrap gap-2 mt-4">
              <Button
                onClick={() => onRecommendationAction?.('approve')}
              ...
              <Button
                onClick={() => onRecommendationAction?.('schedule_meetup')}
                variant="outline"
              >
              ...
              <Button
                onClick={() => onRecommendationAction?.('request_more_info')}
                variant="outline"
              >
