...
import type { VirtualMeetup } from '@pawfectmatch/core';
...
type VirtualMeetupFormData = z.infer<typeof virtualMeetupSchema>;

interface VirtualMeetupSchedulerProps {
  petId: string;
  applicantId: string;
  shelterId: string;
  applicationId: string;
  onSchedule: (data: VirtualMeetupFormData) => Promise<void>;
  existingMeetups?: VirtualMeetup[];
}

export const VirtualMeetupScheduler = ({
  petId,
  applicantId,
  shelterId,
  applicationId,
  onSchedule,
  existingMeetups = [],
}: VirtualMeetupSchedulerProps) => {
  const [isScheduling, setIsScheduling] = useState(false);
  const [agendaItems, setAgendaItems] = useState<string[]>(['']);
  const [selectedPlatform, setSelectedPlatform] = useState<
    'zoom' | 'google_meet' | 'phone' | 'video_call'
  >('zoom');
  ...
  const updateAgendaItem = (index: number, value: string) => {
    ...
    setValue(
      'agenda',
      newItems.filter((item) => item.trim() !== ''),
    );
  };

  const onFormSubmit = async (data: VirtualMeetupFormData) => {
          ...
          >
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div className="space-y-2">
                ...
                <Select
                  ...
                  onValueChange={(value: string) => setValue('duration', parseInt(value, 10))}
                >
                  <SelectTrigger>
            ...
            <div className="space-y-2">
              ...
              <Select
                ...
                onValueChange={(value: 'zoom' | 'google_meet' | 'phone' | 'video_call') => {
