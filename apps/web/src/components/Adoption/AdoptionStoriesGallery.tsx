...
import type { AdoptionSuccessStory } from '@pawfectmatch/core';
import { Calendar, Heart, Quote, Share2, Star } from 'lucide-react';
import { useState } from 'react';

interface AdoptionStoriesGalleryProps {
  stories: AdoptionSuccessStory[];
  onLoadMore?: () => void;
  hasMore?: boolean;
  isLoading?: boolean;
}

export const AdoptionStoriesGallery = ({
  stories,
  onLoadMore,
  hasMore = false,
  isLoading = false,
}: AdoptionStoriesGalleryProps) => {
  const [selectedStory, setSelectedStory] = useState<AdoptionSuccessStory | null>(null);
  ...
  const StoryCard = ({ story, onClick }: { story: AdoptionSuccessStory; onClick: () => void }) => (
    <Card
    ...
    >
      <div className="relative">
        <img
          src={story.photos[0] || '/placeholder-pet.jpg'}
          alt={`${story.petId} adoption story`}
  ...
  const StoryModal = ({ story, onClose }: { story: AdoptionSuccessStory; onClose: () => void }) => {
        ...
        <div className="space-y-6">
                  ...
                  {story.photos.map((photo, index) => (
                    <CarouselItem key={index}>
                      <div className="relative aspect-video">
                        <img
                          src={photo}
                          alt={`Adoption story photo ${index + 1}`}
          ...
          <div className="flex border-b">
            <button
              ...
              className={`px-4 py-2 font-medium ${activeTab === 'story'
            ...
            {story.followUpUpdates && story.followUpUpdates.length > 0 && (
              <button
          ...
          <div className="min-h-[200px]">
            {activeTab === 'story' ? (
              ...
              <div className="space-y-4">
                {story.followUpUpdates?.map((update, index) => (
                  <Card key={index}>
                    <CardContent className="p-4">
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Featured Stories */}
      {stories.filter((story) => story.featured).length > 0 && (
        <div className="space-y-4">
