...
export default function StoriesCarousel({
  ...
  const { onTouchStart, onTouchMove, onTouchEnd } = useAdvancedGestures(
    { swipeThreshold: 50 },
    {
      onSwipe: (event) => {
  ...
  if (!currentStory || !stories.length) return null;
        ...
        <div className="relative w-full h-full max-w-md mx-auto">
          {currentStory.type === 'photo' ? (
            <Image
              ...
              alt={`${currentStory.petName}'s story`}
              fill
      ...
      <AnimatePresence>
        {showReplyInput && (
          <motion.div
            ...
            className="absolute bottom-0 left-0 right-0 z-20 p-4 bg-black/80 backdrop-blur-sm"
          >
            <div className="flex items-center space-x-3">
              <input
                ref={inputRef}
                ...
                className="flex-1 px-4 py-3 bg-white/10 border border-white/20 rounded-full text-white placeholder-white/60 focus:outline-none focus:ring-2 focus:ring-primary-500"
                onKeyPress={(e) => {
                  if (e.key === 'Enter') {
                    handleReply();
                  }
              ...
              <button
                ...
                className="px-6 py-3 bg-primary-500 text-white rounded-full font-semibold hover:bg-primary-600 disabled:opacity-50 disabled:cursor-not-allowed transition-colors"
              >
                Send
              </button>
            </div>
            ...
            <div className="flex flex-col space-y-2">
              {reactions.slice(-3).map((reaction, index) => (
                <motion.div
                  ...
                  className="w-12 h-12 bg-black/50 rounded-full flex items-center justify-center text-2xl"
                >
                  {reaction}
                </motion.div>
              ))}
...
export function useStories() {
  ...
  const addStory = (story: Omit<Story, 'id' | 'timestamp' | 'isViewed' | 'views' | 'reactions' | 'replies'>) => {
  ...
  const addReaction = (storyId: string) => {
    setStories(prev => 
      prev.map(story => 
        story.id === storyId 
