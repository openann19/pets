...
export function StoryViewer({
    ...
    const viewStoryMutation = useMutation({
        mutationFn: async (storyId: string) => {
            const response = await http.post<ViewStoryResponse>(
    ...
    const replyMutation = useMutation({
        mutationFn: async ({ storyId, message }: { storyId: string; message: string }) => {
            const response = await http.post<ReplyToStoryResponse>(
                ...
                <div className="relative w-full h-full max-w-[500px] flex flex-col">
                    ...
                    <div className="absolute top-12 left-0 right-0 z-20 px-4 flex items-center justify-between">
                        <div className="flex items-center gap-3">
                            <Image
                                src={currentGroup.user.profilePhoto || '/default-avatar.png'}
                            ...
                            {!showReplyInput ? (
                                ...
                                >
                                    <input
                                        ...
                                        className="flex-1 px-4 py-3 bg-white/10 backdrop-blur-sm rounded-full text-white placeholder-white/60 focus:outline-none focus:ring-2 focus:ring-white/30"
                                        autoFocus
                                    />
                                    <button
                                        onClick={handleReplySubmit}
                                        disabled={!replyText.trim() || replyMutation.isPending}
                                        className="p-3 bg-purple-500 rounded-full text-white hover:bg-purple-600 transition disabled:opacity-50 disabled:cursor-not-allowed"
                                    >
                                        <Send size={20} />
                                    </button>
                                </motion.div>
