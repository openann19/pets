      ...
      >
        ...
        <div className={`absolute inset-0 flex items-center justify-center ${sizes.avatar} rounded-full overflow-hidden`}>
          <Image
            src={petAvatar}
            alt={petName}
            width={size === 'sm' ? 40 : size === 'md' ? 56 : size === 'lg' ? 72 : 88}
            height={size === 'sm' ? 40 : size === 'md' ? 56 : size === 'lg' ? 72 : 88}
...
interface StoriesRowProps {
  ...
  onStoryClick: (petId: string) => void;
  className?: string;
}

export function StoriesRow({ stories, onStoryClick, className = '' }: StoriesRowProps) {
  return (
    <div className={`flex space-x-4 overflow-x-auto pb-4 ${className}`}>
      {stories.map((story) => (
        <StoryRing
          ...
          petAvatar={story.petAvatar}
          hasUnseenStories={story.hasUnseenStories}
          storyCount={story.storyCount}
          onClick={() => onStoryClick(story.petId)}
          size="md"
          showCount={true}
        />
      ))}
...
export function useStoryRings() {
  const [storyRings, setStoryRings] = useState<Array<{
    petId: string;
    petName: string;
    petAvatar: string;
    hasUnseenStories: boolean;
  ...
  const updateStoryRing = useCallback((petId: string, updates: {
    hasUnseenStories?: boolean;
    storyCount?: number;
  }) => {
    setStoryRings(prev => 
  ...
  const markStoriesAsSeen = useCallback((petId: string) => {
