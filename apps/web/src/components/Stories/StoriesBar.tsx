...
export function StoriesBar({ onStoryClick, onCreateClick }: StoriesBarProps) {
    ...
    return (
        <div className="relative">
            ...
            <div
                ref={scrollRef}
                className="flex gap-4 overflow-x-auto pb-4 scrollbar-hide scroll-smooth px-4"
            >
                <AnimatePresence mode="popLayout">
                    {data.map((group: StoryGroup, index: number) => (
                        <motion.div
                            key={group.userId}
                        ...
                        >
                            ...
                            <div className="relative">
                                {/* Gradient Ring */}
                                <div
                                    className={`absolute inset-0 rounded-full p-[2px] ${group.hasUnseen
                                ...
                                <div className="relative w-16 h-16 rounded-full overflow-hidden border-2 border-white dark:border-gray-900 transform transition-transform duration-200 group-hover:scale-105">
                                    <Image
                                        src={group.user.profilePhoto || '/default-avatar.png'}
                                        alt={group.user.name}
                                        fill
                                ...
                                {/* Story Count Badge */}
                                {group.storyCount > 1 && (
                                    <div className="absolute -bottom-1 -right-1 bg-purple-500 text-white text-xs font-bold rounded-full w-5 h-5 flex items-center justify-center border-2 border-white dark:border-gray-900">
                                        {group.storyCount}
                                    </div>
                                )}
                            </div>
