...
interface EmojiReactionsProps {
  isVisible: boolean;
  onReaction: (emoji: string, position: { x: number; y: number }) => void;
  onClose: () => void;
  className?: string;
}
...
export default function EmojiReactions({
  ...
  const [activeCategory, setActiveCategory] = useState<keyof typeof EMOJI_CATEGORIES>('love');
  ...
  return (
    ...
    >
        ...
        {reactions.map((reaction) => (
          ...
          >
            ...
            <motion.div
              ...
              className="absolute inset-0 text-4xl text-white/30"
              style={{
                filter: 'blur(2px)'
              }}
            >
      ...
      {/* Emoji picker */}
      <AnimatePresence>
        {showPicker && tapPosition && (
          <motion.div
            ref={pickerRef}
            initial={{ opacity: 0, scale: 0.8, y: 20 }}
            animate={{ opacity: 1, scale: 1, y: 0 }}
            exit={{ opacity: 0, scale: 0.8, y: 20 }}
            className="absolute bg-black/80 backdrop-blur-sm rounded-2xl p-4 shadow-2xl"
            style={{
            ...
            <div className="grid grid-cols-6 gap-2">
              {EMOJI_CATEGORIES[activeCategory].map((emoji) => (
                <motion.button
                  ...
                  className="p-2 text-2xl hover:scale-110 transition-transform rounded-lg hover:bg-white/10"
...
export function useEmojiReactions() {
  ...
  const addReaction = (emoji: string, position: { x: number; y: number }, userId: string, userName: string) => {
    const reaction: Reaction = {
      id: `reaction_${Date.now()}_${Math.random().toString(36).substr(2, 9)}`,
      emoji,
      position,
