...
class UltraTestSuite {
  ...
  private async testAuthenticationFlow(): Promise<void> {
    ...
    await this.runTest('Get Current User Profile', async () => {
      ...
      if (!result.success) throw new Error(result.error || 'Failed to get user');
      return result.data;
    });
  ...
  private async testAllEndpoints(): Promise<void> {
    ...
    await this.runTest('Get All Pets', async () => {
      ...
      if (!result.success) throw new Error(result.error || 'Failed to get pets');
      return { count: result.data?.length || 0 };
    });

    await this.runTest('Get My Pets', async () => {
      const result = await apiClient.getMyPets();
      if (!result.success) throw new Error(result.error || 'Failed to get my pets');
      return { count: result.data?.length || 0 };
    });
    ...
    await this.runTest('Create New Pet', async () => {
      ...
      if (!result.success) throw new Error(result.error || 'Pet creation failed');
      return result.data;
    });
    ...
    await this.runTest('Get Swipe Queue', async () => {
      ...
      if (!result.success) throw new Error(result.error || 'Failed to get swipe queue');
      return { count: result.data?.length || 0 };
    });
    ...
    await this.runTest('Perform Swipe Action', async () => {
      ...
      if (!result.success) throw new Error(result.error || 'Swipe failed');
      return result.data;
    });

    await this.runTest('Get Matches', async () => {
      const result = await apiClient.getMatches();
      if (!result.success) throw new Error(result.error || 'Failed to get matches');
      return { count: result.data?.length || 0 };
    });
    ...
    await this.runTest('Update Location', async () => {
      ...
      if (!result.success) throw new Error(result.error || 'Location update failed');
      return result.data;
    });

    await this.runTest('Get Nearby Pets', async () => {
      const result = await apiClient.getNearbyPets(10);
      if (!result.success) throw new Error(result.error || 'Failed to get nearby pets');
      return { count: result.data?.length || 0 };
    });
  ...
  private async testAIServices(): Promise<void> {
    ...
    await this.runTest('AI Bio Generation', async () => {
      ...
      if (!result.success) throw new Error(result.error || 'Bio generation failed');
      return { bioLength: result.data?.bio?.length || 0 };
    });

    await this.runTest('AI Photo Analysis', async () => {
      const result = await apiClient.analyzePhoto(
    ...
    await this.runTest('AI Compatibility Calculation', async () => {
      ...
      if (!result.success) throw new Error(result.error || 'Compatibility calculation failed');
      return result.data;
    });
    ...
    await this.runTest('AI Profile Suggestions', async () => {
      ...
      if (!result.success) throw new Error(result.error || 'Profile suggestions failed');
      return result.data;
    });
  }
...
if (typeof window !== 'undefined') {
  (window as any).runUltraTests = () => ultraTestSuite.runAllTests();
