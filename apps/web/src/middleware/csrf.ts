...
function requiresCsrfProtection(request: NextRequest): boolean {
    ...
    if (!CSRF_CONFIG.protectedMethods.includes(method as typeof CSRF_CONFIG.protectedMethods[number])) {
        return false;
    }
 ...
 */
function hasValidAuthorization(request: NextRequest): boolean {
...
export async function csrfMiddleware(request: NextRequest): Promise<NextResponse | null> {
    ...
    if (critical) {
        if (!hasAuth) {
            console.warn('[CSRF] Critical route missing authorization:', { pathname });
            return NextResponse.json(
                {
                    error: 'Unauthorized',
        ...
        if (!cookieToken || !headerToken || !validateCsrfToken(cookieToken, headerToken)) {
            console.warn('[CSRF] Critical route CSRF validation failed:', { pathname });
            return NextResponse.json(
                {
    ...
    if (!cookieToken) {
        console.warn('[CSRF] Missing CSRF cookie:', { pathname });
        return NextResponse.json(
            {
    ...
    if (!headerToken) {
        console.warn('[CSRF] Missing CSRF header:', { pathname });
        return NextResponse.json(
            {
                error: 'CSRF token missing',
    ...
    console.log('[CSRF] Validation passed:', { pathname, method: request.method });
    return null; // Allow request to proceed
}
