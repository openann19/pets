...
import { _geofencingService } from '@/services/GeofencingService';
...
const MapPage = (): JSX.Element => {
  ...
  interface Notification {
    id: string;
    type: 'zone_enter' | 'zone_exit' | 'match_nearby' | 'general';
    title: string;
    message: string;
    timestamp: string;
  ...
  useEffect(() => {
    ...
    const initializeServices = async (): Promise<void> => {
      try {
        const initialized = await _geofencingService.initialize();
        if (!initialized) return;
        logger.info('Geofencing service initialized');

        _geofencingService.subscribe(
          'map-page',
        ...
        // Store cleanup function for the useEffect to return
        cleanupFunction = () => {
          _geofencingService.unsubscribe('map-page');
          window.removeEventListener('geofence-notification', handleNotification as EventListener);
        };
      } catch (e) {
        logger.warn('Geofencing init skipped in dev', { error: e });
        return;
      }
    };
  ...
  // Real-time stats updates using actual data
  useEffect(() => {
      ...
      try {
        ...
        if (response.ok) {
          ...
          setStats({
            ...
            nearbyMatches: data.nearbyMatches || 0,
            recentActivity: data.recentActivity || 0,
          });
        } else {
          // Fallback to pins count
          setStats({
            totalPets: pins.length,
            activePets: pins.length,
            nearbyMatches: pins.length,
            recentActivity: pins.length,
          });
        }
      } catch (error) {
        logger.error('Failed to fetch map stats', { error });
        // Fallback to pins count
        setStats({
          totalPets: pins.length,
          activePets: pins.length,
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    {activityTypes.map((activity) => (
                      <button
                        ...
                        className={`p-3 rounded-xl border-2 transition-all duration-200 ${filters.activityTypes.includes(activity.id)
          ...
          <div className="flex-1 relative">
            ...
            <AIMapFeatures
              ...
              onInsightClick={(insight) => {
                logger.info('AI Insight clicked', { insight });
                ...
                <div className="p-6">
                      ...
                      notifications.map((notification, index) => (
                        ...
                        >
                          <div className="flex items-start space-x-3">
                            <div
                              className={`w-8 h-8 rounded-full flex items-center justify-center text-sm ${notification.priority === 'high'
                  ...
                  {notifications.length > 0 && (
                    <button
                      onClick={() => {
                        setNotifications([]);
                        _geofencingService.clearNotifications();
                      }}
